...
(*VVV This should be configurable for each site, or allow to specify the port
  for each link/form?? Same for hostname ... *)
      ...
      let make_full_uri
          ...
          getparams : uri =
        Pages.uri_of_string
          (make_full_string_uri ?https ~service ~sp ?fragment getparams)
...
(*VVV We trust current protocol? *) 
      ...
      let a 
        ...
        let proto_prefix = make_proto_prefix ~sp https in
(*VVV We trust current protocol? *) 
        match get_kind_ service with
        | `Attached attser ->
            (let suff, params_string =
...
(*VVV We trust current protocol? *) 
      ...
      let get_form ?https ?a ~service ~sp ?fragment f =
        get_form_ (fun x f -> f x) (fun x -> x) 
          ?https ?a ~service ~sp ?fragment f

      let lwt_get_form ?https ?a ~service ~sp ?fragment f =
        get_form_ Lwt.bind Lwt.return ?https ?a ~service ~sp ?fragment f
...
(*VVV We trust current protocol? *) 
      ...
      let post_form 
          ?https ?a ~service ~sp ?fragment ?keep_get_na_params f getparams =
      ...
      let make_uri ?https ~service ~sp ?fragment gp =
        Pages.uri_of_string (make_string_uri ?https ?fragment ~service ~sp gp)
