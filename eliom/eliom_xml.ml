...
let map_separator = function
  | XML.Space -> Eliom_client_types.Space
  | XML.Comma -> Eliom_client_types.Comma

let map_attrib = function
  | XML.AFloat (string, float) -> Eliom_client_types.AFloat (string, float)
  | XML.AInt (string, int) -> Eliom_client_types.AInt (string, int)
  | XML.AStr (string1, string2) -> Eliom_client_types.AStr (string1, string2)
  | XML.AStrL (separator, string, string_list) ->
    Eliom_client_types.AStrL (map_separator separator, string, string_list)

let rec map_node t n =
  if (T.find t n).sent
  then (Eliom_client_types.Ref (make_node_id_ t n), None)
    (* we don't need to attach id to reference node *)
  else
    let node = match n.XML.elt with
      | XML.Empty -> Eliom_client_types.Empty
      | XML.Comment s -> Eliom_client_types.Comment s
      | XML.EncodedPCDATA s -> Eliom_client_types.EncodedPCDATA s
      | XML.PCDATA s -> Eliom_client_types.PCDATA s
      | XML.Entity s -> Eliom_client_types.Entity s
      | XML.Leaf (e,a) -> Eliom_client_types.Leaf (e,List.map map_attrib a)
      | XML.Node (e,a,l) -> Eliom_client_types.Node (e,List.map map_attrib a,List.map (map_node t) l)
    in
    (node, (T.find t n).id)
