...
(*****************************************************************************)
(* this section should be in eliom_sessions.ml *)

let appl_name_key = Polytables.make_key ()
let appl_instance_key = Polytables.make_key ()
...
let get_eliom_appl_nlp_ ~sp =
  ...
  let (a, b, c) as d =
    match get_non_localized_get_parameters sp eliom_appl_nlp
    with
      | None -> None, None, false
      | Some (a, b) -> Some a, Some b, true
  in
  Polytables.set ~table:rc ~key:appl_name_key ~value:a;
  Polytables.set ~table:rc ~key:appl_instance_key ~value:b;
...
let get_application_instance ~sp =
  let esp = Eliom_sessions.esp_of_sp sp in
  let rc = esp.Eliom_common.sp_request.Ocsigen_extensions.request_info.Ocsigen_extensions.ri_request_cache in
  try 
    Polytables.get ~table:rc ~key:appl_instance_key
  with Not_found -> 
...
let _ =
  Eliom_sessions.get_application_name_ := get_application_name;
  Eliom_sessions.get_application_instance_ := get_application_instance;
  Eliom_sessions.get_content_only_ := get_content_only
