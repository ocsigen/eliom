...
For developing an Eliom application, we recommend that you use our [PPX syntax extension](./ppx-syntax.md).
...
Client-side parts are using [Lwt](https://ocsigen.org/lwt/.html), enabling concurrency in the browser very easily.
...
Module [`Eliom.Comet`](./eliom.server/Eliom-Comet.md) allows the server to send notifications to the client (even if the client is not explicitely doing a request). Use module `Eliom.React` to make client-server reactive programming (using the [React](http://erratique.ch/software/react) external library).
