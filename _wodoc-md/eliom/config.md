...
When an exception is raised during the generation of a page, or when the page has not been found or has wrong parameters, an HTTP error 500 or 404 is sent to the client. You may want to catch these exceptions to print your own error page. Do this using `Eliom.Service.set_exn_handler`. Here is the handler used by the tutorial:

```ocaml
let _ = Eliom.Service.set_exn_handler
   (fun e -> match e with
...
```
Use [`Eliom.Config.get_config`](./eliom.server/Eliom-Config.md#val-get_config) during the initialization of your module to get the data between <!--wodoc:span class="code"-->\<eliom<!--wodoc:end-->\> and <!--wodoc:span class="code"-->\</eliom<!--wodoc:end-->\>.
