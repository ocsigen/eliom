...
```
`items` is a list of pairs correlating the services to be linked with the text to be displayed: `home`, `info`, and `tutorial` are our three services (generated, for example, by [`Eliom.Registration.App.create`](./eliom.server/Eliom-Registration-App.md#val-create) or [`Eliom.Service.create`](./eliom.server/Eliom-Service.md#val-create)).
