...
val get_form : 
  ...
  service:
    (_, unit, Service.get, _, _, _, _, _, 'gn, _, Service.non_ocaml) Service.t ->
  ?hostname:string ->
  ?port:int ->
  ?fragment:string ->
  ?keep_nl_params:[ `All | `Persistent | `None ] ->
  ...
  [> Html_types.form ] elt
```
The function `get_form service formgen` creates a GET `<form>` to `service`. The content of the `<form>` is generated by the function `formgen`, that takes the names of the service parameters as parameters. By default, the `action` attribute is a relative URL recomputed at each request with [`make_uri`](./Eliom-Content_sigs-module-type-LINKS_AND_FORMS.md#val-make_uri).

By default, the form is realized such that the client-side Eliom application keeps running irrespectable of the usage of the form (cf. `Client.change_page`).
...
NB that the default value of `~xhr` is configurable through [`Config.set_default_links_xhr`](./Eliom-Config.md#val-set_default_links_xhr).

The optional parameter `~a` allows one to add extra HTML attributes to the generated node.

See [`make_uri`](./Eliom-Content_sigs-module-type-LINKS_AND_FORMS.md#val-make_uri) for description of other optional parameters.
...
val post_form : 
  ...
  ?a:[< Html_types.form_attrib ] attrib list ->
  service:
    ('get, _, Service.post, _, _, _, _, _, _, 'pn, Service.non_ocaml) Service.t ->
  ?hostname:string ->
  ?port:int ->
  ?fragment:string ->
  ?keep_nl_params:[ `All | `Persistent | `None ] ->
...
See `Service.make` for a description of the `~keep_get_na_params` optional parameter ; see [`get_form`](./#val-get_form) for `~xhr` and see [`make_uri`](./Eliom-Content_sigs-module-type-LINKS_AND_FORMS.md#val-make_uri) for other optional parameters.
