...
```
Values of type `caml_event_handler` represents event handler build with the `{{ ... }}` syntax (see the Eliom manual for more information on `syntax extension`). Such values are expected by functions like `Content.Html.a_onclick`.
