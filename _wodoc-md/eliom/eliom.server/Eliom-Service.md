...
# Module `Eliom.Service`

Creation and manipulation of Eliom services.

See the Eliom manual for a detailed introduction to the concept of [Eliom services](./../server-services.md).

See [`create`](./#val-create) for the main service creation function.x


### Services

See [`Service.create`](./#val-create) (on the server) for how to create services.
...
- `'gn` is the type of GET parameters names. See [`Parameter.param_name`](./Eliom-Parameter.md#type-param_name) and form generation functions (e. g. `Content.Html.D.get_form` ).
- `'pn` is the type of POST parameters names. See [`Parameter.param_name`](./Eliom-Parameter.md#type-param_name) and form generation functions (e. g. `Content.Html.D.post_form` ).
- ` 'ret` is an information on what the service returns. See [`Registration.kind`](./Eliom-Registration.md#type-kind).
```ocaml
and result = 
  | No_contents
...
The function `create ~id ~path ()` creates a service ([`Service_sigs.S.t`](./Eliom-Service_sigs-module-type-S.md#type-t)) identified as per `path` and accepting parameters as per `meth`. See [`Service_sigs.S.path_option`](./Eliom-Service_sigs-module-type-S.md#type-path_option) and [`Service_sigs.TYPES.meth`](./Eliom-Service_sigs-module-type-TYPES.md#type-meth) for the respective arguments.
...
If the optional argument `~keep_nl_params:`Persistent` (resp. `~keep_nl_params:`All`) is given, all links towards that service will keep persistent (resp. all) non localized GET arguments of the current service. The default is ``None`. See the eliom manual for more information about [non localized parameters](./../server-params.md#nonlocalizedparameters).
...
If the optional `~csrf_safe` argument is `true`, we create a ["CSRF-safe" service](./../server-security.md#csrf). In that case the `~name` argument is ignored. The default is `false`.

The `~csrf_scope` and `~csrf_secure`, if present, should respectively correspond to the `~scope` and `~secure` arguments that will be given to the associated `register` function. Otherwise the registration will fail with `Service.Wrong_session_table_for_CSRF_safe_coservice`. See [`Registration.Html.register`](./Eliom-Registration-Html.md#val-register), [`Registration.App.register`](./Eliom-Registration-App.md#val-register) or any other [`Registration`](./Eliom-Registration.md)`.*.register` functions for a description of these arguments.

*Warning: `create` must be called when the site information is available, that is, either during a request or during the initialisation phase of the site. Otherwise, it will raise the exception [`Common.Site_information_not_available`](./Eliom-Common.md#exception-Site_information_not_available). If you are using static linking, you must delay the call to this function until the configuration file is read, using [`Service.register_eliom_module`](./#val-register_eliom_module). Otherwise you will also get this exception.*
...
val create_attached_get : 
  ...
  ('gp,
    ...
    t
```
`create_attached_get ~fallback ~get_params ()` attaches a new service on the path of `fallback`. The new service implements the GET method and accepts `get_params`, in addition to an automatically-generated parameter that is used to identify the service and does not need to be provided by the programmer. `fallback` remains available. For a description of the optional parameters see [`create`](./#val-create).
...
```
`create_attached_post ~fallback ~post_params ()` attaches a new service on the path of `fallback`. The new service implements the POST method and accepts the GET parameters of `fallback`, in addition to the POST parameters `post_params`. An automatically-generated parameter is used to identify the service and does not need to be provided by the programmer. `fallback` remains available. For a description of the optional parameters see [`create`](./#val-create).
...
val unregister : 
  ...
  unit
```
The function `unregister service` unregister the service handler previously associated to `service` with [`Registration.Html.register`](./Eliom-Registration-Html.md#val-register), [`Registration.App.register`](./Eliom-Registration-App.md#val-register) or any other [`Registration`](./Eliom-Registration.md)`.*.register` functions. See the documentation of those functions for a description of the `~scope` and `~secure` optional parameters.
