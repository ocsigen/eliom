...
```
returns the sub path of the URL using the type `Lib.Url.path`. The sub-path is the full path without the path of the site (set in the configuration file).
...
```
returns the root of the site. Raises `Common.Site_information_not_available` if unavailable.
