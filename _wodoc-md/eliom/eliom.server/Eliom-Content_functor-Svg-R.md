...
```
```ocaml
type 'a elt = 'a Content_core.Svg.elt
```
SVG elements.
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom-Shared_content-Svg-R-Xml.md) for details.
...
```ocaml
type 'a attrib = 'a Content_core.Svg.attrib
```
SVG attributes
