...
If you want to restrict the visibility of an Eliom reference or a service: \* to a browser session, use `~scope:Common.default_session_scope`, \* to a group of sessions, use `~scope:Common.default_group_scope`, \* to a client process, use `~scope:Common.default_process_scope`. If you have a client side Eliom program running, and you want to restrict the visibility of the service to this instance of the program, use `~scope:Common.default_process_scope`.

You can create new scope hierarchies with [`Common.create_scope_hierarchy`](./#val-create_scope_hierarchy). Thus it is possible to have for example several sessions that can be opened or closed independently. They use different cookies.
...
In particular, you cannot use the function before the configuration file is read for example when you are using *static linking*. In that case you must delay the function call using [`Service.register_eliom_module`](./Eliom-Service.md#val-register_eliom_module).
