...
The type variables in [`elt`](./Eliom-Content_core-Svg-Make.md#type-elt) and [`attrib`](./Eliom-Content_core-Svg-Make.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Eliom-Content_core-Svg-Make.md#val-tot) and [`toelt`](./Eliom-Content_core-Svg-Make.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
