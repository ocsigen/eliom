...
# Module `Eliom.Mod_sessexpl`

```ocaml
val iter_service_cookies : 
  ((string * Common.tables Common.Service_cookie.t) -> unit Lwt.t) ->
  unit Lwt.t
```
```ocaml
val iter_data_cookies : 
  ((string * Common.Data_cookie.t) -> unit Lwt.t) ->
  unit Lwt.t
```
```ocaml
val iter_persistent_cookies : 
  ((string * Mod_cookies.cookie) -> unit Lwt.t) ->
  unit Lwt.t
```
```ocaml
val fold_service_cookies : 
  ((string * Common.tables Common.Service_cookie.t) -> 'a -> 'a Lwt.t) ->
  'a ->
  'a Lwt.t
```
```ocaml
val fold_data_cookies : 
  ((string * Common.Data_cookie.t) -> 'a -> 'a Lwt.t) ->
  'a ->
  'a Lwt.t
```
```ocaml
val fold_persistent_cookies : 
  ((string * Mod_cookies.cookie) -> 'a -> 'a Lwt.t) ->
  'a ->
  'a Lwt.t
```
```ocaml
