...
let myservice =
  Service.create
    ~path:(Service.Path ["foo"])
    ~meth:(Service.Get (Parameter.any))
    ()

let () =
  Registration.Html.register ~service:myservice
    (fun get_params () ->
      Lwt.return
         Content.Html.F.(html (head (title (txt "")))
                                    (body [h1 [txt "Hello"]])))
```
Inserting a link towards that service, with parameters:

```ocaml
Content.Html.D.a ~service:myservice [txt "Home"] [("param1", "v1"); ("param2", "v2")]
```
...
```ocaml
let%server r = Reference.eref ~scope:Common.default_session_scope 0

let%server f () =
  let%lwt v = Reference.get r in
  Reference.set r (v + 1);
  ...
...
- `Common.default_session_scope` (different value for each browser),
- `Common.default_process_scope` (different value for each tab),
- `Common.default_group_scope` (different value for each user),
- `Common.site_scope` (value for the whole site),
- `Common.global_scope` (global value for the whole server). Eliom references are persistant if you add optional parameter `~persistent` to function `Reference.eref`.
