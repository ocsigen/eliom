...
```
Record an (external) JavaScript file to be included in [`Tools.F.html`](./Eliom-Tools-F.md#val-html).

```ocaml
val with_css_file : string list -> unit
```
Record an CSS file to be included in [`Tools.F.html`](./Eliom-Tools-F.md#val-html).
