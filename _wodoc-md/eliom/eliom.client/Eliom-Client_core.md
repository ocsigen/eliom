...
```
```ocaml
module Client_value_registry : sig ... end
```
```ocaml
...
```
```ocaml
val global_data : compilation_unit_global_data Lib.String_map.t ref
```
```ocaml
val do_next_server_section_data : 
  compilation_unit_id:Lib.String_map.key ->
  unit
```
```ocaml
...
val rebuild_node_svg : 
  string ->
  'a Content_core.Svg.F.elt ->
  'b Js_of_ocaml__Js.t
```
```ocaml
val rebuild_node : string -> 'a Content_core.Html.F.elt -> 'b Js_of_ocaml__Js.t
```
