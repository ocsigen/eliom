...
```
Returns the name of currently running Eliom application, defined while applying `Registration.App` functor.
...
```
A `('a, 'b) server_function` provides transparently access to a server side function which has been created by [`Client.server_function`](./#type-server_function).

See also [the opaque server side representation](./#type-server_function).

The handling of exception on the server corresponds to that of [`Client.call_ocaml_service`](./#val-call_ocaml_service).
