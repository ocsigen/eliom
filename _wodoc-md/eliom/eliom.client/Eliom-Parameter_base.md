...
type (_, _) params_type_ = 
  ...
  | TCoord : string -> (coordinates, [ `One of coordinates ] param_name)
                       params_type_
  | TFile : string -> (Lib.file_info, [ `One of Lib.file_info ] param_name)
                      params_type_
  | TUserType : (string * 'a Common.To_and_of_shared.t) -> ('a,
                                                           [ `One of 'a ]
                                                             param_name)
                                                           params_type_
  ...
  | TESuffixu : (string * 'a Common.To_and_of_shared.t) -> ('a,
                                                           [ `One of 'a ]
                                                             param_name)
                                                           params_type_
...
val file : 
  string ->
  (Lib.file_info, [ `One of Lib.file_info ] param_name) params_type_
```
```ocaml
...
```ocaml
val construct_params_string : (string * Mod_parameters.field) list -> string
```
```ocaml
val construct_params_list : 
  ...
  ('a, [< suff ], 'b) params_type ->
  'a ->
  string list option * (string * Mod_parameters.field) list
```
```ocaml
...
```ocaml
val remove_from_nlp : 
  'a 'c. 'b Lib.String.Table.t ->
  ('a, [< suff ], 'c) params_type ->
  'b Lib.String.Table.t
```
```ocaml
type nl_params_set = (string * Mod_parameters.param) list Lib.String.Table.t
```
```ocaml
val empty_nl_params_set : 'a Lib.String.Table.t
```
```ocaml
...
```
```ocaml
type files = (string * Lib.file_info) list
```
```ocaml
