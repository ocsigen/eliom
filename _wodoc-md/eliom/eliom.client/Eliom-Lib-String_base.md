...
```
`contains s c` is [`String.contains_from`](./Eliom-Lib-String.md#val-contains_from)` s 0 c`.
...
```
`index s c` is [`String.index_from`](./Eliom-Lib-String.md#val-index_from)` s 0 c`.

```ocaml
val index_opt : string -> char -> int option
```
`index_opt s c` is [`String.index_from_opt`](./Eliom-Lib-String.md#val-index_from_opt)` s 0 c`.

since 4\.05
```ocaml
val rindex : string -> char -> int
```
`rindex s c` is [`String.rindex_from`](./Eliom-Lib-String.md#val-rindex_from)` s (length s - 1) c`.

```ocaml
val rindex_opt : string -> char -> int option
```
`rindex_opt s c` is [`String.rindex_from_opt`](./Eliom-Lib-String.md#val-rindex_from_opt)` s (length s - 1) c`.
...
```
Same as [`Stdlib.String.starts_with`](./../../ocaml-compiler/stdlib/Stdlib-String.md#val-starts_with).
