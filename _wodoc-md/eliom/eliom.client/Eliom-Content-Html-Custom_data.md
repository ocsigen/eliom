...
val create : 
  ...
  'a t
```
Create a custom data field by providing string conversion functions. If the `default` is provided, calls to [`Content.Html.Custom_data.get_dom`](./#val-get_dom) return that instead of throwing an exception `Not_found`.
