...
The type variables in [`elt`](./Eliom-Eliom_form-Make_links-argument-1-H.md#type-elt) and [`attrib`](./Eliom-Eliom_form-Make_links-argument-1-H.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Eliom-Eliom_form-Make_links-argument-1-H.md#val-tot) and [`toelt`](./Eliom-Eliom_form-Make_links-argument-1-H.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.

Note that some implementation may not be iterable or printable, such as the Dom representation exposed by js\_of\_ocaml.

```ocaml
module W : Xml_wrap.T with type 'a t = 'a with type 'a tlist = 'a list
```
```ocaml
type 'a wrap = 'a W.t
```
