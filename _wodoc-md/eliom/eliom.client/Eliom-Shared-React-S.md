...
val accum : 
  ...
  'a React.signal
```
`accum e i` is `S.hold i (`[`E.accum`](./Eliom-Shared-React-E.md#val-accum)` e i)`.
...
val fold : 
  ...
  'a React.signal
```
`fold f i e` is `S.hold i (`[`E.fold`](./Eliom-Shared-React-E.md#val-fold)` f i e)`.
...
Given an equality function `equal` and a type `t`, the functor [`Make`](./Eliom-Shared-React-S-Make.md) automatically applies the `eq` parameter of the combinators. The outcome is combinators whose *results* are signals with values in `t`.

Basic types are already specialized in the module [`Special`](./Eliom-Shared-React-S-Special.md), open this module to use them.

```ocaml
module type EqType = sig ... end
```
Input signature of [`Make`](./Eliom-Shared-React-S-Make.md)

```ocaml
module type S = sig ... end
```
Output signature of [`Make`](./Eliom-Shared-React-S-Make.md)
