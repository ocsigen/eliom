...
```
Abstract type for service parameters. See for example the parameter `~get_param` of `Service.Http.service`.
...
- ` 'c` is the type of the parameter name, usually an instance of [`Parameter.param_name`](./Eliom-Parameter.md#type-param_name), as used by forms construction functions (e.g., the last parameter of `Content.Html.D.get_form`), and specialized form widget (see for example the section `Eliom_form widget` of `Content.HTML5.D`). )
...
val file : 
  string ->
  (Lib.file_info, [ `WithoutSuffix ], [ `One of Lib.file_info ] param_name)
    params_type
```
...
```
The service takes a list of parameters. The first parameter of this function is the name of the list. The service handler will receive a list of values. To create the form, an iterator of type [`Parameter.listnames`](./Eliom-Parameter.md#type-listnames) is given to generate the name for each value.
