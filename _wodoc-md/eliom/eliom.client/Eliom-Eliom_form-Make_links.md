...
For XML tree build with TyXML, like [`Html`](./Eliom-Eliom_form-module-type-Html.md) or `Svg.F`, the function `f` is applied each time the XML tree is sent to the client (either as page content or as a marshalled OCaml value). Hence, the function is always evaluated in the context of a service handler.
