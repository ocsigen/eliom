...
let nullsum =
  begin
    let module E = (val Enum_nullsum.make : Enum with type a = nullsum) in
...
let poly0 =
  begin
    let module E = (val Enum_poly0.make : Enum with type a = poly0) in
...
let t =
  begin
    ListLabels.iter (Enum.enum_from_to<int> (-1000) 1000)
      ~f:(fun i ->
            assert (Enum.succ<t> i = i+1);
            assert (Enum.pred<t> i = i-1);
            assert (Enum.to_enum<t> i = i);
            assert (Enum.from_enum<t> i = i));
  end
