...
object (self : 'self)
  ...
  method tagspec t =
    self#crush (match t with
                  | Tag (_, tys) -> List.map self#expr tys
...
object (self : 'self)
  ...
  method tagspec = function
    | Tag (name, tys) -> Tag (name, List.map (self # expr) tys)
    | Extends e -> Extends (self # expr e)
end

let free_tvars =
  let o = object
     inherit [ExprSet.t] fold as default
     method crush = List.fold_left ExprSet.union ExprSet.empty
     method poly_expr (params, e) =
       List.fold_left (fun s p -> ExprSet.remove (`Param p) s) (default#expr e) params
     method expr = function
       | `Param _ as e  -> ExprSet.singleton e
       | e -> default#expr e
  end in o#expr
...
struct
  ...
  let build_alias_map m =
    build_subst (List.concat_map (function None -> [] | Some e -> [e]) m)

  let rec decl : Ast.ctyp -> decl list * subst = function
    ...
    fun ctyp ->
      let decls, aliases = List.split (List.map decl (split ctyp)) in
        List.concat
          (List.map
             (List.map
                (substitute_decl (NameMap.union_disjoint aliases))) decls)
end
...
module Untranslate (C:sig val _loc : Camlp4.PreCast.Ast.Loc.t end) =
struct
  open Camlp4.PreCast
  open C
  ...
  and tagspec = function
      | Tag (c, []) -> <:ctyp< `$c$ >>
      | Tag (c, tys) -> <:ctyp< `$c$ of $expr (`Tuple tys)$ >>
