  ...
  let make = (module (Defaults (struct
	...
	*)
    ...
    and from_stream stream =
      ...
      in aux zero 0
  end)) : Dump with type a = P.t)
end

module Dump_int32 = Dump_intN (Int32)
module Dump_int64 = Dump_intN (Int64)
module Dump_nativeint = Dump_intN (Nativeint)
module Dump_int = struct

  let make = (module (Defaults(struct
    type a = int
    let to_buffer buffer int =
      (let module M = (val Dump_nativeint.make : Dump with type a = nativeint)
      in M.to_buffer) buffer (Nativeint.of_int int)
    and from_stream stream =
      Nativeint.to_int
	((let module M = (val Dump_nativeint.make : Dump with type a = nativeint)
	  in M.from_stream) stream)
  end)) : Dump with type a = int)

end

module Dump_char = struct

  let make = (module (Defaults (struct
    type a = char
    let to_buffer = Buffer.add_char
    and from_stream = Stream.next
  end)) : Dump with type a = char)

end

(* This is questionable; it doesn't preserve sharing *)
module Dump_string = struct

  let make = (module (Defaults (struct
    type a = string
    let to_buffer buffer string =
      begin
        (let module M = (val Dump_int.make : Dump with type a = int)
	 in M.to_buffer) buffer (String.length string);
        Buffer.add_string buffer string
      end
    and from_stream stream =
      let len =
	(let module M = (val Dump_int.make : Dump with type a = int)
	 in M.from_stream) stream in
      let s = String.create len in
        for i = 0 to len - 1 do
          String.set s i (Stream.next stream) (* could use String.unsafe_set here *)
        done;
        s
  end)) : Dump with type a = string)

end

module Dump_float = struct

  let make = (module (Defaults (
  struct
    type a = float
    let to_buffer buffer f =
      (let module M = (val Dump_int64.make : Dump with type a = int64)
       in M.to_buffer) buffer (Int64.bits_of_float f)
    and from_stream stream =
      Int64.float_of_bits
	((let module M = (val Dump_int64.make : Dump with type a = int64)
          in M.from_stream) stream)
  end)) : Dump with type a = float)

end

(* This should end up a bit more compact than the derived version *)
module Dump_list = struct

  let make (type t_a) (m_a : (module Dump with type a = t_a)) =
    (module (Defaults (struct
	 ...
	 tail-recursive) *)
      ...
      let to_buffer buffer items =
        (let module M = (val Dump_int.make : Dump with type a = int)
	 in M.to_buffer) buffer (List.length items);
        List.iter ((let module M = (val m_a : Dump with type a = t_a)
	            in M.to_buffer) buffer) items
      and from_stream stream =
	let rec aux items = function
          | 0 -> items
          | n -> aux ((let module M = (val m_a : Dump with type a = t_a)
	               in M.from_stream) stream :: items) (n-1)
	in List.rev
	  (aux [] ((let module M = (val Dump_int.make : Dump with type a = int)
	            in M.from_stream) stream))
    end)) : Dump with type a = t_a list)
...
module Dump_option = struct
  ...
  let make (type t_a) (m_a : (module Dump with type a = t_a)) =
	...
	| None   -> (let module M = (val Dump_int.make : Dump with type a = int)
	            in M.to_buffer) buffer 0
	| Some s ->
            (let module M = (val Dump_int.make : Dump with type a = int)
	            in M.to_buffer) buffer 1;
            (let module M = (val m_a : Dump with type a = t_a)
	            in M.to_buffer) buffer s
      and from_stream stream =
	match (let module M = (val Dump_int.make : Dump with type a = int)
	       in M.from_stream) stream with
        | 0 -> None
        | 1 -> Some ((let module M = (val m_a : Dump with type a = t_a)
	              in M.from_stream) stream)
        | i      -> bad_tag i stream "option"
    end)) : Dump with type a = t_a option)

end

module Dump_bool = struct
  ...
  end)) : Dump with type a = bool)

end

module Dump_unit = struct

  let make = (module (Defaults (struct
    type a = unit
    let to_buffer _ () = ()
    and from_stream _ = ()
  end)) : Dump with type a = unit)
...
module Dump_undumpable (P : sig type a val tname : string end) = struct

  let make = (module (Defaults (struct
    type a = P.a
    let to_buffer _ _ =
      failwith ("Dump: attempt to serialise a value of unserialisable type : "
		^ P.tname)
    let from_stream _ =
      failwith ("Dump: attempt to deserialise a value of unserialisable type : "
		^ P.tname)
...
module Dump_via_marshal (P : sig type a end) = struct

  let make = (module (Defaults (struct
     (* Rather inefficient. *)
    include P
    let to_buffer buffer obj =
      Buffer.add_string buffer (Marshal.to_string obj [Marshal.Closures])
