...
module Defaults'
    ...
    (B : sig val make: (module Bounded with type a = E.a) end): Enum with type a = E.a =
  struct
    include E
    let firstCon = let module B = (val B.make : Bounded with type a = E.a) in B.min_bound
    let lastCon =  let module B = (val B.make : Bounded with type a = E.a) in B.max_bound
...
module Enum_bool = struct
  let make = (module (Defaults(struct
    type a = bool
    let numbering = [false, 0; true, 1]
  end)) : Enum with type a = bool)
end

module Enum_char = struct
  let make = (module (Defaults'(struct
    type a = char
    let from_enum = Char.code
    let to_enum = Char.chr
  end)(Bounded_char)) : Enum with type a = char)
end

module Enum_int = struct
  let make = (module (Defaults' (struct
    type a = int
    let from_enum i = i
    let to_enum i = i
  end)(Bounded_int)) : Enum with type a = int)
end
...
module Enum_unit = struct
  ...
  end)(Bounded_unit)) : Enum with type a = unit)
end
