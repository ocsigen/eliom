...
(** Primitive instances for Pickle **)
...
module Repr : sig
  ...
  type t = Bytes of string | CApp of (int option * Id.t list)
      deriving (Dump, Show)
...
struct
  ...
  module Utils (T : Deriving_Typeable.Typeable) (E : Deriving_Eq.Eq with type a = T.a) =
  struct
...
module Read : sig
  type s = (repr * (Deriving_Typeable.dynamic option)) IdMap.t
  include Deriving_monad.Monad_state_type with type state = s
  val find_by_id : id -> (Repr.t * Deriving_Typeable.dynamic option) m
...
struct
  ...
  module Utils (T : Deriving_Typeable.Typeable) = struct
...
struct
  ...
  type ('a,'b) pair = 'a * 'b
	deriving (Dump)
  type capp = int option * Id.t list
	deriving (Dump)
  ...
  *)
  ...
  type discriminated_dumpable = Id.t * discriminated
	deriving (Dump)
...
module Pickle_from_dump = struct

  let make (type t_a)
      (m_p : (module Deriving_Dump.Dump with type a = t_a))
      (m_e : (module Deriving_Eq.Eq with type a = t_a))
      (m_t : (module Deriving_Typeable.Typeable with type a = t_a))
	...
	open Write
	let pickle obj =
	  let module T = (val m_t: Deriving_Typeable.Typeable with type a = t_a) in
	  let module E = (val m_e: Deriving_Eq.Eq with type a = t_a) in
	  let module W = Utils(T)(E) in
	  W.allocate obj
            (fun id -> W.store_repr id (Repr.of_string ((let module P = (val m_p: Deriving_Dump.Dump with type a = t_a) in P.to_string) obj)))
	...
	let unpickle id =
	  ...
	  find_by_id id >>= fun (repr, dynopt) ->
            match dynopt with
            | None ->
		let obj : a = (let module P = (val m_p: Deriving_Dump.Dump with type a = t_a) in P.from_string) (Repr.to_string repr) in
                W.update_map id obj >>
                return obj
            | Some obj -> return ((let module T = (val m_t: Deriving_Typeable.Typeable with type a = t_a) in T.throwing_cast) obj)
   end)) : Pickle with type a = t_a)

end

module Pickle_unit = struct
  let make = Pickle_from_dump.make
      Deriving_Dump.Dump_unit.make
      Deriving_Eq.Eq_unit.make
      Deriving_Typeable.Typeable_unit.make
end
module Pickle_bool = struct
  let make = Pickle_from_dump.make
      Deriving_Dump.Dump_bool.make
      Deriving_Eq.Eq_bool.make
      Deriving_Typeable.Typeable_bool.make
end
module Pickle_char = struct
  let make = Pickle_from_dump.make
      Deriving_Dump.Dump_char.make
      Deriving_Eq.Eq_char.make
      Deriving_Typeable.Typeable_char.make
end
module Pickle_int = struct
  let make = Pickle_from_dump.make
      Deriving_Dump.Dump_int.make
      Deriving_Eq.Eq_int.make
      Deriving_Typeable.Typeable_int.make
end
(* module Pickle_num = struct *)
  (* let make = Pickle_from_dump.make *)
      (* Deriving_Dump.Dump_num.make *)
      (* Deriving_Eq.Eq_num.make *)
      (* Deriving_Typeable.Typeable_num.make *)
(* end *)
module Pickle_float = struct
  let make = Pickle_from_dump.make
      Deriving_Dump.Dump_float.make
      Deriving_Eq.Eq_float.make
      Deriving_Typeable.Typeable_float.make
end
module Pickle_string = struct
  let make = Pickle_from_dump.make
      Deriving_Dump.Dump_string.make
      Deriving_Eq.Eq_string.make
      Deriving_Typeable.Typeable_string.make
end

module Pickle_option = struct
  let make (type t_a) (m_a : (module Pickle with type a = t_a)) =
    (module Defaults(struct

      type a = t_a option

      let typeable =
	let module M = (val m_a : Pickle with type a = t_a) in
	Deriving_Typeable.Typeable_option.make M.typeable

      let eq =
	let module M = (val m_a : Pickle with type a = t_a) in
	Deriving_Eq.Eq_option.make M.eq

      open Write
      let pickle =
	let module T = (val typeable : Deriving_Typeable.Typeable with type a = t_a option) in
	let module E = (val eq : Deriving_Eq.Eq with type a = t_a option) in
	let module W = Utils(T)(E) in
	let rec pickle o = match o with
        | None as obj ->
            W.allocate obj
        ...
        | Some v0 as obj ->
            W.allocate obj
	      (fun thisid ->
                (let module M = (val m_a : Pickle with type a = t_a)
		 in M.pickle) v0 >>= fun id0 ->
	...
	in pickle
      ...
      let unpickle =
	let module T = (val typeable : Deriving_Typeable.Typeable with type a = t_a option) in
	let module R = Utils(T) in
	let f = function
          | 0, [] -> return None
          | 1, [id] ->
	      (let module M = (val m_a : Pickle with type a = t_a)
	       in M.unpickle) id >>= fun obj -> return (Some obj)
          | n, _ -> raise (UnpicklingError
                             ("Unexpected tag encountered unpickling "
                              ^"option : " ^ string_of_int n)) in
        R.sum f
    end) : Pickle with type a = t_a option)
end

module Pickle_list = struct
  ...
  let make (type t_a) (m_a: (module Pickle with type a = t_a)) =
    (module (Defaults (struct
      ...
      let typeable =
	let module M = (val m_a : Pickle with type a = t_a) in
	Deriving_Typeable.Typeable_list.make M.typeable

      let eq =
	let module M = (val m_a : Pickle with type a = t_a) in
	Deriving_Eq.Eq_list.make M.eq

      open Write
      let pickle =
	let module T =
	  (val typeable : Deriving_Typeable.Typeable with type a = t_a list) in
	let module E = (val eq : Deriving_Eq.Eq with type a = t_a list) in
	let module W = Utils(T)(E) in
	let rec pickle l = match l with
	| [] as obj ->
            W.allocate obj
              (fun this -> W.store_repr this (Repr.make ~constructor:0 []))
	| (v0::v1) as obj ->
            W.allocate obj (fun this ->
	      (let module M = (val m_a : Pickle with type a = t_a)
	       in M.pickle) v0 >>= fun id0 ->
	      pickle v1 >>= fun id1 ->
	      W.store_repr this (Repr.make ~constructor:1 [id0; id1]))
	in pickle
	...
	let f = function
	  ...
	  | n, _ -> raise (UnpicklingError
                             ("Unexpected tag encountered unpickling "
                              ^"option : " ^ string_of_int n)) in
	W.sum f id
    end)) : Pickle with type a = t_a list)
end

module Pickle_ref = struct

  let make (type tm_a) (m_a : (module Pickle with type a = tm_a)) =

    (module Defaults(struct
      type a = tm_a ref

      let typeable =
	let module M = (val m_a : Pickle with type a = tm_a)
        in Deriving_Typeable.Typeable_ref.make M.typeable

      let eq =
	let module M = (val m_a : Pickle with type a = tm_a)
        in Deriving_Eq.Eq_ref.make M.eq

      open Write
      let pickle =
        let module Eq = (val eq : Deriving_Eq.Eq with type a = tm_a ref) in
        let module Typeable =
          (val typeable : Deriving_Typeable.Typeable with type a = tm_a ref) in
        let module W = Utils(Typeable)(Eq) in
        let rec pickle (obj) =
          W.allocate obj (fun this ->
            ((let module M = (val m_a : Pickle with type a = tm_a)
              in M.pickle) obj.contents) >>= (fun contents ->
                W.store_repr this (Repr.make [ contents ])))
        in pickle
	...
	let module W = Utils(Typeable) in
        W.record (fun self -> function
          | [ contents ] ->
              ((let module M = (val m_a : Pickle with type a = tm_a)
              in M.unpickle) contents) >>= (fun contents ->
                (self.contents <- contents; return self))
          | _ -> raise (UnpicklingError
			  "unexpected object encountered unpickling ref"))
          1

    end) : Pickle with type a = tm_a ref)
