...
module InContext (C : sig val context : Base.context val _loc : Camlp4.PreCast.Loc.t end) =
struct
  open C
  open Type
  open Utils
...
*)
  ...
  and expr : Type.expr -> Ast.expr = function
	...
	let qname =
	  try
	    [runtimename ; List.assoc qname predefs]
          ...
          (List.fold_right2
             (fun t n (p,e) ->
                let v = Printf.sprintf "t%d" n in
                  ...
                  Ast.ExCom (_loc, <:expr< $expr t$ $lid:v$ >>, e))
             ts
...
let _ = Base.register classname
  ((fun (_loc, context, decls) ->
     let module F = InContext(struct let _loc = _loc and context = context end) in
