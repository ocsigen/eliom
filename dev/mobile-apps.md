...
- Define JavaScript variables `__eliom_server` and `__eliom_app_name` in the `index.html` of your mobile app, to set the main server and the name of the app. See `Eliom.Client.init_client_app` and the default `eliom.html` file generated by `Ocsigen Start`.
- Use function [`Eliom.Client.is_client_app`](./eliom.server/Eliom-Client.md#val-is_client_app) to test whether you are running a mobile app (an app that was started client-side). `Ocsigen Start` also has a class `os-client-app` on the `html` tag.
