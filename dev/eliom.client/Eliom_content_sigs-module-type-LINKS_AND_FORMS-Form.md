...
val get_form : 
  ...
  [> Html_types.form ] elt
```
The function `get_form service formgen` creates a GET `<form>` to `service`. The content of the `<form>` is generated by the function `formgen`, that takes the names of the service parameters as parameters. By default, the `action` attribute is a relative URL recomputed at each request with [`make_uri`](./Eliom_content_sigs-module-type-LINKS_AND_FORMS.md#val-make_uri).

By default, the form is realized such that the client-side Eliom application keeps running irrespectable of the usage of the form (cf. [`Eliom_client.change_page`](./Eliom_client.md#val-change_page)).
...
See [`make_uri`](./Eliom_content_sigs-module-type-LINKS_AND_FORMS.md#val-make_uri) for description of other optional parameters.
...
See `Eliom_service.make` for a description of the `~keep_get_na_params` optional parameter ; see [`get_form`](./#val-get_form) for `~xhr` and see [`make_uri`](./Eliom_content_sigs-module-type-LINKS_AND_FORMS.md#val-make_uri) for other optional parameters.
