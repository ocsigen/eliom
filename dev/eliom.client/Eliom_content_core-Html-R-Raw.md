...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom_content_core-Html-R-Raw-Xml.md) for details.
...
module Xml : 
  Xml_sigs.T
    with type 'a W.t = 'a Xml_wed.W.t
    with type 'a W.tlist = 'a Xml_wed.W.tlist
    with type ('a, 'b) W.ft = ('a, 'b) Xml_wed.W.ft
    with type uri = Xml_wed.uri
    with type event_handler = Xml_wed.event_handler
    with type mouse_event_handler = Xml_wed.mouse_event_handler
    with type keyboard_event_handler = Xml_wed.keyboard_event_handler
    with type touch_event_handler = Xml_wed.touch_event_handler
    with type attrib = Xml_wed.attrib
    with type elt = Xml_wed.elt
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom_content_core-Html-R-Raw-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom_content_core-Html-R-Raw-Unsafe.md#val-string_attrib).
