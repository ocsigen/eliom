...
The type variables in [`elt`](./Eliom_form-Make-argument-1-H.md#type-elt) and [`attrib`](./Eliom_form-Make-argument-1-H.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Eliom_form-Make-argument-1-H.md#val-tot) and [`toelt`](./Eliom_form-Make-argument-1-H.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
