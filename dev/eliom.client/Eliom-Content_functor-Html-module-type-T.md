...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom-Content_functor-Html-module-type-T-Xml.md) for details.
...
module Xml : 
  Xml_sigs.T
    with type 'a W.t = 'a Content_core.Xml.W.t
    with type 'a W.tlist = 'a Content_core.Xml.W.tlist
    with type ('a, 'b) W.ft = ('a, 'b) Content_core.Xml.W.ft
    with type uri = Content_core.Xml.uri
    with type event_handler = Content_core.Xml.event_handler
    with type mouse_event_handler = Content_core.Xml.mouse_event_handler
    with type keyboard_event_handler = Content_core.Xml.keyboard_event_handler
    with type touch_event_handler = Content_core.Xml.touch_event_handler
    with type attrib = Content_core.Xml.attrib
    with type elt = Content_core.Xml.elt
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom-Content_functor-Html-module-type-T-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom-Content_functor-Html-module-type-T-Unsafe.md#val-string_attrib).
...
```ocaml
val svg : 
  ?a:[< Html_types.svg_attrib ] Content_core.Svg.attrib list ->
  [< Html_types.svg_content ] Content_core.Svg.elt list_wrap ->
  [> Html_types.svg ] elt
```
...
The optional parameter `keep_nl_params` allows one to override the `keep_nl_params` parameter used when creating the `service`, see `Service.create` for a detailed description.
...
For XML tree build with TyXML, like [`Html`](./Eliom-Content_functor-Html.md) or [`Svg.F`](./Eliom-Content_core-Svg-F.md), the function `f` is applied each time the XML tree is sent to the client (either as page content or as a marshalled OCaml value). Hence, the function is always evaluated in the context of a service handler.
...
If the CSS is generated by an Eliom service, use [`make_uri`](./#val-make_uri) to calculate the service URI. If the CSS is a static file, you may also use [`Service.static_dir`](./Eliom-Service.md#val-static_dir) or [`Service.extern`](./Eliom-Service.md#val-extern) to abstract the file with a service.
...
If the script content is generated by an Eliom service, use [`make_uri`](./#val-make_uri) to calculate the service URI. If it is a static file, you may also use [`Service.static_dir`](./Eliom-Service.md#val-static_dir) or [`Service.extern`](./Eliom-Service.md#val-extern) to abstract the file with a service.
...
val a : 
  ...
  service:
    ('get, unit, Service.get, _, _, _, _, _, _, unit, Service.non_ocaml)
      Service.t ->
  ?hostname:string ->
  ?port:int ->
  ?fragment:string ->
...
By default, the link is implemented in a way that allows the client-side Eliom application to keep running, irrespective of the usage of the link (cf. [`Client.change_page`](./Eliom-Client.md#val-change_page)).

By contrast, if the optional parameter `~xhr:false` is given, the link is realized as a standard HTML link and clicking it discontinues the Eliom application. The `~xhr` parameter has no effect outside an Eliom application. The default value of `~xhr` is configurable through `Config.set_default_links_xhr`.
