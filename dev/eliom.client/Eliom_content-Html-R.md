...
See [`Html_sigs.T`](./../../tyxml/tyxml.functor/Html_sigs-module-type-T.md), If you want to create an untyped form, you will have to use [`Eliom_content.Html.D.Raw`](./Eliom_content-Html-D-Raw.md) otherwise, use the form module. For more information, see [the manual](./../server-links.md#forms).
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom_content-Html-R-Xml.md) for details.
...
module Xml : 
  Xml_sigs.T
    with type 'a W.t = 'a Eliom_content_core.Xml_wed.W.t
    with type 'a W.tlist = 'a Eliom_content_core.Xml_wed.W.tlist
    with type ('a, 'b) W.ft = ('a, 'b) Eliom_content_core.Xml_wed.W.ft
    with type uri = Eliom_content_core.Xml_wed.uri
    with type event_handler = Eliom_content_core.Xml_wed.event_handler
    ...
    with type attrib = Eliom_content_core.Xml_wed.attrib
    with type elt = Eliom_content_core.Xml_wed.elt
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom_content-Html-R-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom_content-Html-R-Unsafe.md#val-string_attrib).
