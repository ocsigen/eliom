...
```ocaml
module Raw = Content_core.Html.F.Raw
```
See [`Html_sigs.T`](./../../tyxml/tyxml.functor/Html_sigs-module-type-T.md).

```ocaml
type +'a elt = 'a Content_core.Html.elt
```
HTML elements.
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom-Content_core-Html-F-Xml.md) for details.
...
```ocaml
type +'a attrib = 'a Content_core.Html.attrib
```
HTML attributes
...
```ocaml
module Xml = Content_core.Html.F.Xml
```
...
```ocaml
module Info = Content_core.Html.F.Info
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom-Content_core-Html-F-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom-Content_core-Html-F-Unsafe.md#val-string_attrib).
...
```
```ocaml
module Unsafe = Content_core.Html.F.Unsafe
```
Unsafe features.
