...
The type variables in [`elt`](./Eliom_content-Html-R-Raw.md#type-elt) and [`attrib`](./Eliom_content-Html-R-Raw.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Eliom_content-Html-R-Raw.md#val-tot) and [`toelt`](./Eliom_content-Html-R-Raw.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
...
module W : 
  Xml_wrap.T
    with type 'a t = 'a Eliom_content_core.Xml_wed.W.t
    with type 'a tlist = 'a Eliom_content_core.Xml_wed.W.tlist
    with type ('a, 'b) ft = ('a, 'b) Eliom_content_core.Xml_wed.W.ft
```
```ocaml
type 'a wrap = 'a W.t
```
...
```
```ocaml
type mouse_event_handler = Eliom_content_core.Xml_wed.mouse_event_handler
```
```ocaml
type keyboard_event_handler = Eliom_content_core.Xml_wed.keyboard_event_handler
```
```ocaml
type touch_event_handler = Eliom_content_core.Xml_wed.touch_event_handler
```
```ocaml
