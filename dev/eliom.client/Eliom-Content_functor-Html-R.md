...
```ocaml
val node : 'a Content_core.Html.elt React.signal -> 'a Content_core.Html.elt
```
...
```ocaml
val filter_attrib : 
  'a Content_core.Html.attrib ->
  bool React.signal ->
  'a Content_core.Html.attrib
```
```ocaml
module Raw = Content_core.Html.R.Raw
```
```ocaml
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom-Content_core-Html-R-Xml.md) for details.
...
```ocaml
module Xml = Content_core.Html.R.Xml
```
...
```ocaml
module Info = Content_core.Html.R.Info
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom-Content_core-Html-R-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom-Content_core-Html-R-Unsafe.md#val-string_attrib).
...
```ocaml
val svg : 
  ?a:[< Html_types.svg_attrib ] Content_core.Svg.R.Raw.attrib list ->
  [< Html_types.svg_content ] Content_core.Svg.R.Raw.elt list_wrap ->
  [> Html_types.svg ] elt
```
...
```
```ocaml
module Unsafe = Content_core.Html.R.Unsafe
```
Unsafe features.
