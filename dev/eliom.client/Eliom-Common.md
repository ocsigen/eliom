...
```
```ocaml
type full_state_name = Common_base.full_state_name = {
...
```
```ocaml
module Full_state_name_table = Common_base.Full_state_name_table
```
```ocaml
type att_key_serv = Common_base.att_key_serv = 
  | SAtt_no
...
```
```ocaml
type na_key_serv = Common_base.na_key_serv = 
  | SNa_no
  | SNa_void_keep
  | SNa_void_dontkeep
  | SNa_get_ of string
...
```
```ocaml
type att_key_req = Common_base.att_key_req = 
  | RAtt_no
  | RAtt_named of string
  | RAtt_anon of string
```
```ocaml
type na_key_req = Common_base.na_key_req = 
  | RNa_no
  | RNa_get_ of string
  | RNa_post_ of string
  | RNa_get' of string
...
```
```ocaml
type client_process_info = Common_base.client_process_info = {
  cpi_ssl : bool;
  cpi_hostname : string;
  cpi_server_port : int;
...
```
```ocaml
type sess_info = Common_base.sess_info = {
...
```
```ocaml
type eliom_js_page_data = Common_base.eliom_js_page_data = {
...
```
```ocaml
val site_init_compilation_unit_id : string
```
```ocaml
...
```
```ocaml
type ('a, 'b) foundornot = ('a, 'b) Common_base.foundornot = 
  | Found of 'a
  | Notfound of 'b
```
...
```
```ocaml
type 'a dircontent = 'a Common_base.dircontent = 
  | Vide
  | Table of 'a direlt ref Lib.String.Table.t
```
```ocaml
and 'a direlt = 'a Common_base.direlt = 
...
```
```ocaml
type page_table_key = Common_base.page_table_key = {
...
```
```ocaml
type ('params, 'result) service = ('params, 'result) Common_base.service = {
...
```
```ocaml
type 'a to_and_of = 'a Common_base.to_and_of = {
  of_string : string -> 'a;
  to_string : 'a -> string;
}
```
...
```
```ocaml
val sitedata : Types.sitedata option ref
```
```ocaml
val global_register_allowed : unit -> (unit -> Types.sitedata) option
```
```ocaml
val get_site_dir : Types.sitedata -> Lib.Url.path
```
```ocaml
val get_site_dir_string : Types.sitedata -> string
```
```ocaml
