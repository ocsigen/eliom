...
```
```ocaml
module Types : Eliom_service_sigs.TYPES
```
...
type ('get, 'post, 'meth, 'attached, 'co, 'ext, 'reg, +'tipo, 'getnames, 'postnames, 'rt)
  ...
  mutable client_fun : ('get -> 'post -> result Lwt.t) option ref
...
val internal_set_client_fun : 
  ...
  ('get -> 'post -> result Lwt.t) Eliom_client_value.t ->
  unit
```
```ocaml
...
```
```ocaml
val default_csrf_scope : 
  [< Eliom_common.user_scope ] option ->
...
```
```ocaml
val no_client_fun : unit -> 'a option ref Eliom_client_value.t option
```
```ocaml
val main_service : 
  https:bool ->
