...
```ocaml
module Raw = Eliom_content_core.Html.F.Raw
```
See [`Html_sigs.T`](./../../tyxml/tyxml.functor/Html_sigs-module-type-T.md).

```ocaml
type +'a elt = 'a Eliom_content_core.Html.elt
```
HTML elements.
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom_content_core-Html-F-Xml.md) for details.
...
```ocaml
type +'a attrib = 'a Eliom_content_core.Html.attrib
```
HTML attributes
...
```ocaml
module Xml = Eliom_content_core.Html.F.Xml
```
...
```ocaml
module Info = Eliom_content_core.Html.F.Info
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom_content_core-Html-F-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom_content_core-Html-F-Unsafe.md#val-string_attrib).
...
```ocaml
val svg : 
  ?a:[< Html_types.svg_attrib ] Eliom_content_core.Svg.attrib list ->
  [< Html_types.svg_content ] Eliom_content_core.Svg.elt list_wrap ->
  [> Html_types.svg ] elt
```
...
```
```ocaml
module Unsafe = Eliom_content_core.Html.F.Unsafe
```
Unsafe features.
...
```
```ocaml
val uri_of_string : (unit -> string) -> Arg.uri
```
```ocaml
val css_link : 
  ?a:[< Html_types.link_attrib ] Arg.attrib list ->
  uri:Arg.uri ->
  unit ->
  [> Html_types.link ] Arg.elt
```
```ocaml
val js_script : 
  ?a:[< Html_types.script_attrib ] Arg.attrib list ->
  uri:Arg.uri ->
  unit ->
  [> Html_types.script ] Arg.elt
```
```ocaml
val a : 
  ?absolute:bool ->
  ?absolute_path:bool ->
  ?https:bool ->
  ?a:[< Html_types.a_attrib ] Arg.attrib list ->
  service:
    ('get,
      unit,
      Eliom_service.get,
