...
See [`Html_sigs.T`](./../../tyxml/tyxml.functor/Html_sigs-module-type-T.md), If you want to create an untyped form, you will have to use [`Content.Html.D.Raw`](./Eliom-Content-Html-D-Raw.md) otherwise, use the form module. For more information, see [the manual](./../server-links.md#forms).
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom-Content-Html-R-Xml.md) for details.
...
module Xml : 
  Xml_sigs.T
    with type 'a W.t = 'a Content_core.Xml_wed.W.t
    with type 'a W.tlist = 'a Content_core.Xml_wed.W.tlist
    with type ('a, 'b) W.ft = ('a, 'b) Content_core.Xml_wed.W.ft
    with type uri = Content_core.Xml_wed.uri
    with type event_handler = Content_core.Xml_wed.event_handler
    with type mouse_event_handler = Content_core.Xml_wed.mouse_event_handler
    with type keyboard_event_handler =
           Content_core.Xml_wed.keyboard_event_handler
    with type touch_event_handler = Content_core.Xml_wed.touch_event_handler
    with type attrib = Content_core.Xml_wed.attrib
    with type elt = Content_core.Xml_wed.elt
```
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom-Content-Html-R-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom-Content-Html-R-Unsafe.md#val-string_attrib).
