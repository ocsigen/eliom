...
Eliom_react.S.Down.of_react`) to achieve the behavior of a client side values which updates alongside with a server correspondent.
...
The notion *server-side client values* allows it to declare and deal with client values within the server-section (and is also supported in the [shared section](#sharedclientvalues)).
