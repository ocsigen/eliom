...
The optional parameter `keep_nl_params` allows one to override the `keep_nl_params` parameter used when creating the `service`, see [`Service.create`](./Eliom-Service.md#val-create) for a detailed description.
...
By default, the link is implemented in a way that allows the client-side Eliom application to keep running, irrespective of the usage of the link (cf. `Client.change_page`).

By contrast, if the optional parameter `~xhr:false` is given, the link is realized as a standard HTML link and clicking it discontinues the Eliom application. The `~xhr` parameter has no effect outside an Eliom application. The default value of `~xhr` is configurable through [`Config.set_default_links_xhr`](./Eliom-Config.md#val-set_default_links_xhr).
