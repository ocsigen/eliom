...
```
`attrib my_data value ` creates a HTML attribute for the custom-data type `my_data` with value `value` for injecting it into an a HTML tree ([`Eliom_content.Html.elt`](./Eliom_content-Html.md#type-elt)).
