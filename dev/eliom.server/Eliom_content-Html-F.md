...
Creation of **F**unctional HTML5 content (copy-able but not referable, see also [`Eliom_content`](./Eliom_content.md)).

### Content creation

See [`Html_sigs.T`](./../../tyxml/tyxml.functor/Html_sigs-module-type-T.md). If you want to create an untyped form, you will have to use [`Eliom_content.Html.F.Raw`](./Eliom_content-Html-F-Raw.md) otherwise, use Eliom form widgets. For more information, see [the manual](./../server-links.md#forms).
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom_content-Html-F-Xml.md) for details.
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom_content-Html-F-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom_content-Html-F-Unsafe.md#val-string_attrib).
...
The optional parameter `keep_nl_params` allows one to override the `keep_nl_params` parameter used when creating the `service`, see [`Eliom_service.create`](./Eliom_service.md#val-create) for a detailed description.
...
For XML tree build with TyXML, like [`Html`](./Eliom_content-Html.md) or [`Svg.F`](./Eliom_content-Svg-F.md), the function `f` is applied each time the XML tree is sent to the client (either as page content or as a marshalled OCaml value). Hence, the function is always evaluated in the context of a service handler.
