...
See [`Html_sigs.T`](./../../tyxml/tyxml.functor/Html_sigs-module-type-T.md). If you want to create an untyped form, you will have to use [`Content.Html.F.Raw`](./Eliom-Content-Html-F-Raw.md) otherwise, use Eliom form widgets. For more information, see [the manual](./../server-links.md#forms).
...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom-Content-Html-F-Xml.md) for details.
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom-Content-Html-F-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom-Content-Html-F-Unsafe.md#val-string_attrib).
...
For XML tree build with TyXML, like [`Html`](./Eliom-Content-Html.md) or [`Svg.F`](./Eliom-Content-Svg-F.md), the function `f` is applied each time the XML tree is sent to the client (either as page content or as a marshalled OCaml value). Hence, the function is always evaluated in the context of a service handler.
