...
Note that the concrete implementation of this type can vary. See [`Xml`](./Eliom_content_core-Html-F-Raw-Xml.md) for details.
...
In the DOM, pointer events inherit from mouse events, so the handlers use [`Xml.mouse_event_handler`](./Eliom_content_core-Html-F-Raw-Xml.md#type-mouse_event_handler).
...
val a_as : 
  ...
  [> `As ] attrib
```
Destination of a preload link. This covers the destinations listed by MDN; other spec destinations can be produced with [`Unsafe.string_attrib`](./Eliom_content_core-Html-F-Raw-Unsafe.md#val-string_attrib).
