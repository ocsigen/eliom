...
The type variables in [`elt`](./Eliom_content_core-Html-Make.md#type-elt) and [`attrib`](./Eliom_content_core-Html-Make.md#type-attrib) are know as *phantom types*. The implementation, defined here, is actually monomorphic.

In particular, tyxml doesn't impose any overhead over the underlying representation. The [`tot`](./Eliom_content_core-Html-Make.md#val-tot) and [`toelt`](./Eliom_content_core-Html-Make.md#val-toelt) functions allows to convert between the typed and the untyped representation without any cost.
