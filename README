 ...
 * js_of_ocaml	     (need version >= 1.0.1)
 ...
 * lwt 	             (need version >= 2.2.1, with react and ssl support)
 * ocamlnet          (tested with 2.2.9 and 3.2)
...
installed on your computer, you may use GODI to install them
automatically, see:

  http://godi.camlcity.org/godi/index.html

For Debian-based linux distributions, see:

  http://ocaml.debian.net/debian/ocaml-3.12.0/

For Fedora-based linux distributions, see:

  http://sympa-roc.inria.fr/wws/arc/caml-list/2011-01/msg00212.html
...
included in your distribution, see:
  ...
  http://ocsigen.org/ocsigenserver/miscserver
 ...
 * run "make depend && make" to compile
 ...
 * [optional] run "make docinstall" as root to install the ocamldoc
 ...
 * run "./server/ocsigen -c ocsigen.conf.local"
   in the ocsigen source directory.

 * open http://localhost:8080/miniwiki in your browser

 * if it does not work, look at the logs (see 'var/log/' in the
 ...
 * sources for this example may be found in the directory
   'examples/miniwiki'. For a full tutorial, see:
     ...
     http://ocsigen.org/manual/dev/intro
     http://localhost:8080/
