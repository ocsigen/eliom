...
module Xml = struct
  ...
  let make_request_node ?(reset = true) elt =
    ...
    if reset
    then f ()
    else match elt.node_id with Runtime.RawXML.NoId -> f () | _ -> elt
  ...
  let set_classes_of_elt elt =
    ...
    | DomNode _ -> failwith "Content_core.set_classes_of_elt"
    | ReactNode _ -> failwith "Content_core.set_classes_of_elt"
    | ReactChildren _ -> failwith "Content_core.set_classes_of_elt"
    | TyXMLNode econtent ->
        { elt with
