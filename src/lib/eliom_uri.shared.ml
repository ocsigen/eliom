...
let reconstruct_relative_url_path_string current_url u suff =
  ...
  if String.length s = 0 then Eliom_common.defaultpagename
  else if s.[0] = '/' then (* possible with optional parameters *) "./" ^ s
  else s
...
let make_proto_prefix ?hostname ?port https : string =
  ...
  let port =
    ...
    | Some p, _ -> p
    | None, Some sp ->
        if https = ssl then Eliom_request_info.get_csp_server_port_sp sp
        else if https then Eliom_config.get_default_sslport ()
        else Eliom_config.get_default_port ()
    | None, None ->
        if https then Eliom_config.get_default_sslport ()
        else Eliom_config.get_default_port ()
  in
...
let make_uri_components_ ?(* does not take into account getparams *) absolute
  ...
  let absolute =
    if absolute || https <> ssl then
      Some (make_proto_prefix ?hostname ?port https)
    else if absolute_path then Some "/"
    else None
  in
  ...
  | Eliom_service.Nonattached naser ->
      ...
      let params' =
        let current_get_params =
          if na_name = Eliom_common.SNa_void_keep then
...
let make_uri_components ?absolute ?absolute_path ?https (type a)
  ...
  let uri =
    ...
    | Some suff ->
        ...
        if uri.[String.length uri - 1] = '/' then uri ^ suff
...
let make_cookies_info (https, service) =
  ...
  let get_path_ (type a)
    ...
    match Eliom_service.info service with
    | Eliom_service.Attached attser ->
        if Eliom_service.is_external service then None
