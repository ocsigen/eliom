...
let get_http_method () =
  ...
  Ocsigen.Request.meth sp.Common.sp_request.request_info

let get_user_agent () =
  let sp = Common.get_sp () in
  match
    Ocsigen.Request.header sp.Common.sp_request.request_info
      Ocsigen_http.Header.Name.user_agent
  with
  | Some ua -> ua
  | None -> ""

let get_full_url_sp sp =
  Uri.to_string (Ocsigen.Request.uri sp.Common.sp_request.request_info)
...
let get_client_conn_to_string () =
  let sp = Common.get_sp () in
  Ocsigen.Request.client_conn_to_string sp.Common.sp_request.request_info

let get_get_params () =
  let sp = Common.get_sp () in
  Ocsigen.Request.get_params sp.Common.sp_request.request_info
...
let get_post_params_sp sp =
  Ocsigen.Request.post_params sp.Common.sp_request.request_info
...
let get_files_sp sp =
  Ocsigen.Request.files sp.Common.sp_request.request_info
    sp.Common.sp_request.request_config.uploaddir
    sp.Common.sp_request.request_config.maxuploadfilesize
...
let get_original_full_path_string_sp sp =
  Ocsigen.Request.original_full_path_string sp.Common.sp_request.request_info
...
let get_current_sub_path_string () =
  let sp = Common.get_sp () in
  Ocsigen.Request.sub_path_string sp.Common.sp_request.request_info

let get_header_hostname () =
  let sp = Common.get_sp () in
  Ocsigen.Request.host sp.Common.sp_request.request_info

let get_timeofday_sp sp =
  Ocsigen.Request.timeofday sp.Common.sp_request.request_info

let get_timeofday () =
  let sp = Common.get_sp () in
  get_timeofday_sp sp

let get_hostname_sp sp = Ocsigen.Extensions.get_hostname sp.Common.sp_request

let get_hostname () =
  let sp = Common.get_sp () in
  get_hostname_sp sp

let get_server_port_sp sp = Ocsigen.Extensions.get_port sp.Common.sp_request

let get_server_port () =
  let sp = Common.get_sp () in
  get_server_port_sp sp

let get_ssl_sp sp = Ocsigen.Request.ssl sp.Common.sp_request.request_info
...
let get_accept_language_sp sp =
  Ocsigen_http.Header.Accept_language.parse
    (Ocsigen.Request.header_multi sp.Common.sp_request.request_info
       Ocsigen_http.Header.Name.accept_language)
...
let get_request_cache_sp sp =
  Ocsigen.Request.request_cache sp.Common.sp_request.request_info
...
let get_cookies ?(cookie_level = `Session) () =
  ...
  | `Session -> Ocsigen.Request.cookies sp.Common.sp_request.request_info
...
let get_ri_sp sp = sp.Common.sp_request.Ocsigen.Extensions.request_info

let get_ri () =
  let sp = Common.get_sp () in
  get_ri_sp sp

let get_tmp_filename fi = fi.Ocsigen.Extensions.tmp_filename
let get_filesize fi = fi.Ocsigen.Extensions.filesize
let get_original_filename fi = fi.Ocsigen.Extensions.raw_original_filename
...
let raw_post_data sp =
  ...
  Lwt.return (Ocsigen.Request.content_type ri, Ocsigen.Request.body ri)
