...
module Timeout = struct
  ...
  let get_global_service_state_timeout ?secure ~cookie_scope () =
    let sitedata =
      Eliom_request_info.find_sitedata "get_global_timeout"
    in
    let secure = Eliom_common.get_secure secure sitedata in
...
let rec close_service_state_if_empty ~scope ?secure () =
  (* Close the session if it has not services inside and no group and
     no sub sessions *)
  ...
  try
      ...
      if
        Eliommod_sessiongroups.Data.group_size
          (sitedata.Eliom_common.site_dir_string, `Client_process,
           Left c.Eliom_common.sc_value) = 0 (* no tab sessions *)
    ...
    | `Session_group scope_hierarchy ->
      (* There is a browser session, we do not close the group, but we
         may close the browser session (this will close the group if
         it is empty). *)
...
let rec close_volatile_state_if_empty ~scope ?secure () =
  (* Close the session if it has not data inside and no group and no
     sub sessions *)
  (* See also in Eliommod_gc and in Eliommod_sessiongroups. *)
  try
    let sp = Eliom_common.get_sp () in
    ...
    let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
    let c =
      Eliommod_datasess.find_data_cookie_only ~cookie_scope ~secure ~sp ()
    in
    match scope with
    | `Session _ ->
      (match !(c.Eliom_common.dc_session_group) with
       ...
       | _ -> ())
    | `Client_process _ -> ()
    (* This should never occure, because we always have tab session
       data when we have a tab session (at least the
       change_page_event). If
       (sitedata.Eliom_common.not_bound_in_data_tables
       c.Eliom_common.dc_value) then
       Eliommod_sessiongroups.Data.remove
       c.Eliom_common.dc_session_group_node *)
    | `Session_group scope_hierarchy ->
      (* There is a browser session, we do not close the group, but we
...
module Group = struct
  ...
  let set_default_max_volatile_data_sessions_per_group
    ...
    in
    let b =
      snd sitedata.Eliom_common.max_volatile_data_sessions_per_group
    in
    if override_configfile || not b
    then
  ...
  let set_default_max_persistent_data_sessions_per_group
    ...
    in
    let b =
      snd sitedata.Eliom_common.max_persistent_data_sessions_per_group
    in
  ...
  let set_default_max_volatile_data_sessions_per_subnet
    ...
    in
    let b =
      snd sitedata.Eliom_common.max_volatile_data_sessions_per_subnet
    in
    if override_configfile || not b
    then
  ...
  let set_default_max_volatile_data_tab_sessions_per_group
    ...
    in
    let b =
      snd sitedata.Eliom_common.max_volatile_data_tab_sessions_per_group
    in
    if override_configfile || not b
    then
  ...
  let set_default_max_volatile_tab_sessions_per_group
      ?override_configfile n =
    set_default_max_service_tab_sessions_per_group
      ?override_configfile n;
    set_default_max_volatile_data_tab_sessions_per_group
      ?override_configfile n
  ...
  let set_default_max_anonymous_services_per_session
    ...
    let sitedata =
      Eliom_request_info.find_sitedata
        "set_default_max_anonymous_services_per_session"
    in
  ...
  let set_default_max_anonymous_services_per_subnet
    ...
    let sitedata =
      Eliom_request_info.find_sitedata
        "set_default_max_anonymous_services_per_subnet"
    in
...
let create_persistent_table ~scope ?secure name : 'a persistent_table =
  let sitedata =
    Eliom_request_info.find_sitedata "create_persistent_table"
  in
  let secure = Eliom_common.get_secure secure sitedata in
...
let get_p_table_key_
  ...
  lwt key =
    ...
    | `Session_group state_name ->
      begin
        match_lwt
          Group.get_persistent_data_session_group
...
let get_table_key_ ~table:(scope, secure, table)
  ...
  | `Session_group state_name ->
    (match
       Group.get_volatile_data_session_group
         ~scope:(`Session state_name) ~secure ()
     with
     | Some a -> a
...
module Discard = struct
  ...
  let discard_all_volatile_data ~scope ?secure () =
    let sitedata =
      Eliom_request_info.find_sitedata "discard_all_volatile_data"
    in
    let secure = Eliom_common.get_secure secure sitedata in
...
module Ext = struct
  ...
  type persistent_cookie_info =
    ...
    (Eliom_common.full_state_name *
     float option (* expiration date by timeout (server side) *) *
  ...
  *)
  ...
  let current_persistent_session_state
      ?secure ?(scope = Eliom_common.default_session_scope) () =
    Eliommod_persess.find_persistent_cookie_only
      ~secure ~cookie_scope:scope () >>= fun cookie ->
    Lwt.return
  ...
  module Low_level = struct
    ...
    (* We have a dynamic scope checking here. Would probably be
       possible to use phantom types again to check this statically. I
       don't want to make the types more complex for now.
  ...
  let get_service_cookie_scope
      ~cookie:(_, ((ct, _, _), _, _, _, _, _)) =
    ct

  let get_volatile_data_cookie_scope
      ~cookie:(_, ((ct, _, _), _, _, _, _)) =
    ct

  let get_persistent_data_cookie_scope
      ~cookie:(_, ((ct, _, _), _, _, _)) =
    ct

  let set_service_cookie_timeout
      ~cookie:(_, (_, _, _, r, _, _)) t =
    match t with
    | None -> r := TNone
    | Some t -> r := TSome t

  let set_volatile_data_cookie_timeout
      ~cookie:(_, (_, _, r, _, _)) t =
    match t with
    | None -> r := TNone
    | Some t -> r := TSome t
  ...
  let get_service_cookie_timeout ~cookie:(_, (_, _, _, r, _, _)) = !r

  let get_volatile_data_cookie_timeout ~cookie:(_, (_, _, r, _, _)) = !r

  let get_persistent_data_cookie_timeout ~cookie:(_, (_, _, r, _)) = r

  let unset_service_cookie_timeout ~cookie:(_, (_, _, _, r, _, _)) =
    r := TGlobal

  let unset_volatile_data_cookie_timeout
      ~cookie:(cookie, (_, _, r, _, _)) =
    r := TGlobal
  ...
  let get_session_group_list () =
    let sitedata =
      Eliom_request_info.find_sitedata "get_session_group_list"
    in
...
let get_volatile_data_cookie ~cookie_scope ?secure () =
  try
    let c =
      Eliommod_datasess.find_data_cookie_only ~cookie_scope ~secure ()
    in
...
let change_pathopt_ sp = function
  ...
  | Some p ->
    (Eliom_request_info.get_sitedata_sp ~sp).Eliom_common.site_dir @ p
