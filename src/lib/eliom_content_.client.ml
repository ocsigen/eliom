...
module MakeManip
    ...
    (Ns: sig
       val content_ns : Eliom_client0.content_ns
     end) = struct
        ...
        let elt' = Kind.toelt elt in
          ...
          | Xml.NoId ->
            Lwt_log.raise_error_f ~section:Lwt_log.eliom
              ~inspect:(Eliom_client0.rebuild_node'
...
module Svg = struct
  ...
  module Id = struct
    ...
    let get_element' id =
      ...
      let node = Eliom_client0.getElementById id in
      Js.Opt.case
...
module Html5 = struct
  ...
  module To_dom = Tyxml_cast.MakeTo(struct
      type 'a elt = 'a F.elt
      let elt x = Js.Unsafe.coerce (Eliom_client0.rebuild_node "n/a" x)
    end)
