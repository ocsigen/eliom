...
let service_state_status ~scope ?secure () =
  ...
  try
    ignore (Eliommod_sersess.find_service_cookie_only ~cookie_scope ~secure ());
    Alive_state
  with
...
let volatile_data_state_status ~scope ?secure () =
  let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
  try
    ignore (Eliommod_datasess.find_data_cookie_only ~cookie_scope ~secure ());
    Alive_state
  with
    | Not_found -> Empty_state
    | Eliom_common.Eliom_Session_expired -> Expired_state
...
let persistent_data_state_status ~scope ?secure () =
  let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
    ...
    (fun () ->
      Eliommod_persess.find_persistent_cookie_only ~cookie_scope ~secure ()
      >>= fun _ ->
      return Alive_state
    )
    (function
...
module Timeout = struct
  ...
  let unset ~kind ~cookie_scope ?secure () =
    try_lwt
      lwt tor =
        match kind with
        | `Service ->
          let c =
            Eliommod_sersess.find_service_cookie_only
              ~cookie_scope ~secure ()
          in
          Lwt.return c.Eliom_common.sc_timeout
        | `Data ->
          let c =
            Eliommod_datasess.find_data_cookie_only
              ~cookie_scope ~secure ()
          in
          Lwt.return c.Eliom_common.dc_timeout
        | `Persistent ->
          lwt c =
            Eliommod_persess.find_persistent_cookie_only
              ~cookie_scope ~secure ()
          in
          Lwt.return c.Eliom_common.pc_timeout
      in
  ...
  let set ~kind ~cookie_scope ?secure t =
    lwt tor =
      match kind with
      | `Service ->
        let c =
    ...
    tor :=
      (match t with
       | None -> Eliom_common.TNone
       | Some t -> Eliom_common.TSome t);
    Lwt.return ()

  let get ~kind ~cookie_scope ?secure () =
    ...
    try_lwt
      lwt tor =
        ...
        | `Service ->
          let c =
            Eliommod_sersess.find_service_cookie_only
              ~cookie_scope ~secure:(Some secure) ~sp ()
          in
          Lwt.return c.Eliom_common.sc_timeout
        | `Data ->
          let c =
...
let rec close_volatile_state_if_empty ~scope ?secure () =
  ...
  try
    ...
    let sitedata = Eliom_request_info.get_sitedata_sp ~sp in
    let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
    let c = Eliommod_datasess.find_data_cookie_only ~cookie_scope ~secure ~sp ()
    in
    match scope with
      | `Session _ ->
...
module Group = struct
  ...
  let set_
    ...
    | None -> ()
    | Some m ->
      Eliommod_sessiongroups.Data.set_max n m
  ...
  let set_persistent
    ...
    let set_max =
      (* FIXME: Eliommod_sessiongroups expects double option; fix
         weird API *)
      match set_max with
      | Some m -> Some (Some m)
      | _ -> None
    ...
    lwt l =
      Eliommod_sessiongroups.Pers.move
        ...
        c.Eliom_common.pc_value !grp n
    in
    lwt () =
      Lwt_list.iter_p
        (Eliommod_persess.close_persistent_state2
           ~scope:(scope:>Eliom_common.user_scope) sitedata None) l
    in
    grp := n;
    Lwt.return ()
  ...
  let set
    ...
    match kind with
    | `Persistent ->
      set_persistent ?set_max ~scope ?secure n
    | `Service | `Data ->
      set_ ?set_max ~scope ?secure ~kind n;
      Lwt.return ()
    ...
    try
      ...
      let n =
        Eliommod_sessiongroups.make_full_group_name
          ~cookie_level:`Session
          (Eliom_request_info.get_request_sp sp)
  ...
  let unset ?set_max ?scope ?secure = function
    | `Service ->
      unset_service ?set_max ?scope ?secure ();
      Lwt.return ()
    | `Data ->
      unset_data ?set_max ?scope ?secure ();
      Lwt.return ()
    | `Persistent ->
      unset_persistent ?scope ?secure ()
  ...
  let get ?scope ?secure kind =
    try_lwt
      match kind with
      | `Service ->
        Lwt.return (get_service ?scope ?secure ())
      | `Data ->
        Lwt.return (get_data ?scope ?secure ())
      | `Persistent ->
        get_persistent ?scope ?secure ()
    with Not_found | Eliom_common.Eliom_Session_expired ->
      Lwt.return None
  ...
  let get_size_service ~scope ?secure () =
    let c =
      Eliommod_sersess.find_service_cookie_only
	~cookie_scope:(scope :> Eliom_common.cookie_scope) ~secure ()
    in
    ...
    | _, _, Right _ ->
      None
    | _, _, Left v ->
      Some (Eliommod_sessiongroups.Serv.group_size
              !(c.Eliom_common.sc_session_group))

  let get_size_data ~scope ?secure () =
    let c =
      Eliommod_datasess.find_data_cookie_only
	~cookie_scope:(scope :> Eliom_common.cookie_scope) ~secure ()
    in
    ...
    | _, _, Right _ -> None
    | _, _, Left v ->
      Some (Eliommod_sessiongroups.Data.group_size
              !(c.Eliom_common.dc_session_group))

  let get_size
      ?(scope = Eliom_common.default_session_scope) ?secure kind =
    try
      match kind with
      | `Service ->
        get_size_service ~scope ?secure ()
      | `Data ->
        get_size_data ~scope ?secure ()
    with Not_found | Eliom_common.Eliom_Session_expired ->
      None

  let set_default_max_per_subnet_service ~override_configfile n =
    let sitedata =
      Eliom_request_info.find_sitedata
        "set_default_max_service_sessions_per_subnet_service"
    in
    let _, b =
  ...
  let set_default_max_per_subnet_data ~override_configfile n =
    let sitedata =
      Eliom_request_info.find_sitedata "set_default_max_per_subnet_data"
    in
    let _, b =
  ...
  let set_default_max_per_subnet ?(override_configfile = false) ~kind n =
    match kind with
    | `Service ->
      set_default_max_per_subnet_service ~override_configfile n
    | `Data ->
      set_default_max_per_subnet_data ~override_configfile n

  let set_max_for_group_or_subnet_service ~scope ?secure m =
    let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
    let c =
      Eliommod_sersess.find_or_create_service_cookie
        ~secure ~cookie_scope ()
  ...
  let set_max_for_group_or_subnet_data ~scope ?secure m =
    let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
    let c =
      Eliommod_datasess.find_or_create_data_cookie
        ~cookie_scope ~secure ()
    ...
    | `Session_group _ ->
      (match
       ...
       with
       | Some (_, node) ->
         Eliommod_sessiongroups.Data.set_max node m
       | _ ->
         ())
    | _ ->
      Eliommod_sessiongroups.Data.set_max
  ...
  let set_max_for_group_or_subnet ~scope ?secure ~kind m =
    match kind with
    | `Service ->
      set_max_for_group_or_subnet_service ~scope ?secure m
    | `Data ->
      set_max_for_group_or_subnet_data ~scope ?secure m
...
(** If the session does not exist, we create it (new cookie, new
    session service table) *)
let get_session_service_table ~sp ~scope ?secure () =
  let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
  let c =
    Eliommod_sersess.find_or_create_service_cookie ~cookie_scope ~secure ~sp ()
  in
  match scope with
    | `Session_group _ ->
      begin
...
let get_session_service_table_if_exists ~sp
  ...
  try
    let c =
      Eliommod_sersess.find_service_cookie_only ~cookie_scope ~secure ~sp ()
    in
    match scope with
      | `Session_group _ ->
	begin
  ...
  lwt key = match scope with
    | `Session_group state_name ->
      begin
        match_lwt
          Group.get
            ~scope:(`Session state_name) ~secure `Persistent
        with
        | Some a -> Lwt.return a
    ...
    | _ ->
      let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
      lwt c = find_cookie ~cookie_scope ~secure:(Some secure) () in
...
let set_persistent_data ~table value =
  let f__ ~cookie_scope ~secure ?sp () =
    Eliommod_persess.find_or_create_persistent_cookie
      ~cookie_scope ~secure ?sp ()
  in
...
let get_table_key_ ~table:(scope, secure, table)
  ...
  (table, match scope with
    | `Session_group state_name ->
      (match
         Group.get_data ~scope:(`Session state_name) ~secure ()
       with Some a -> a
         | _ -> Eliom_common.default_group_name)
    | _ ->
      let cookie_scope = Eliom_common.cookie_scope_of_user_scope scope in
      let c = find_cookie ~cookie_scope ~secure:(Some secure) () in
      c.Eliom_common.dc_value)
...
let set_volatile_data ~table value =
  let f__ ~cookie_scope ~secure ?sp () =
    Eliommod_datasess.find_or_create_data_cookie ~cookie_scope ~secure ?sp ()
  in
  ...
  match secure with
    | None ->
        ...
        Eliommod_persess.close_persistent_state ~scope
          ~secure:(Some false) ()
    | _ ->
        Eliommod_persess.close_persistent_state ~scope ~secure ()
...
let discard_services ~scope ?secure () =
  match secure with
    ...
    | _ ->
        Eliommod_sersess.close_service_state ~scope ~secure ()
  ...
  match secure with
    | None ->
        ...
        Eliommod_datasess.close_data_state
          ~scope ~secure:(Some false) ()
    | _ ->
        Eliommod_datasess.close_data_state ~scope ~secure ()
...
let discard_all_scopes ?secure () =
  let discard_name scope_hierarchy =
    lwt () = discard ?secure ~scope:(`Session_group scope_hierarchy) () in
...
let discard_all ~scope ?secure () =
  lwt () = discard_all_services ~scope ?secure () in
  discard_all_data ~scope ?secure ()

let discard_everything () =
  let discard_name scope_hierarchy =
    lwt () = discard_all ~scope:(`Session_group scope_hierarchy) () in
...
(*VVV Do we need this? + check
  (* The following function returns the group to which belongs a
     session or client process state: *)
...
let number_of_cookies = function
  | `Service ->
    Lwt.return (Eliommod_sessexpl.number_of_service_cookies ())
  | `Data ->
    Lwt.return (Eliommod_sessexpl.number_of_data_cookies ())
  | `Persistent ->
...
let get_volatile_data_cookie ~cookie_scope ?secure () =
  try
    let c = Eliommod_datasess.find_data_cookie_only ~cookie_scope ~secure () in
...
let get_cookie ~cookie_scope ?secure kind =
  match kind with
  | `Service ->
    Lwt.return (get_service_cookie ~cookie_scope ?secure ())
  | `Data ->
    Lwt.return (get_volatile_data_cookie ~cookie_scope ?secure ())
  | `Persistent ->
    get_persistent_data_cookie ~cookie_scope ?secure ()
