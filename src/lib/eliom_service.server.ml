...
let get_or_post_ s = match get_get_or_post s with
  ...
  | `Head -> Ocsigen_http_frame.Http_header.HEAD
  | `Patch -> Ocsigen_http_frame.Http_header.PATCH
  | `Options -> Ocsigen_http_frame.Http_header.OPTIONS
...
module MakeBase = struct
  ...
  let head_service = head_service
  let patch_service = patch_service
  let options_service = options_service
  ...
  let head_coservice = head_coservice
  let patch_coservice = patch_coservice
  let options_coservice = options_coservice
  let coservice' = coservice'
  let post_coservice' = post_coservice'
  let put_coservice' = put_coservice'
  let delete_coservice' = delete_coservice'
  ...
  let external_head_service = external_head_service
  let external_patch_service = external_patch_service
  let external_options_service = external_options_service
end
