...
let gc_timeouted_services now tables =
  let rec aux t filename direltr thr =
    ...
    | Eliom_common.Dir r ->
        ...
        | Eliom_common.Vide -> (
            ...
            | Eliom_common.Table tr ->
                ...
                if String.Table.is_empty newr then t := Eliom_common.Vide
    ...
    | Eliom_common.File ptr ->
        Eliom_common.Serv_Table.fold
          ...
          !ptr return_unit
        >>= fun () ->
        (if Eliom_common.Serv_Table.is_empty !ptr then
           match !t with
           | Eliom_common.Vide -> ()
           | Eliom_common.Table tr ->
               let newr = String.Table.remove filename tr in
  ...
  and empty_one t =
    ...
    | Eliom_common.Table r -> (
        if String.Table.is_empty r then (
          t := Eliom_common.Vide;
          Lwt.return_unit)
        else
...
let gc_timeouted_naservices now tr =
  ...
  | Eliom_common.ATable t ->
      if Eliom_common.NAserv_Table.is_empty t then (
...
let service_session_gc sitedata =
  ...
  | Some t ->
      let rec f () =
        ...
        (if tables.Eliom_common.table_contains_services_with_timeout then
           gc_timeouted_services now tables
         else return_unit)
        >>= fun () ->
        (if tables.Eliom_common.table_contains_naservices_with_timeout then
           gc_timeouted_naservices now tables.Eliom_common.table_naservices
         else return_unit)
        >>= fun () ->
                  ...
                  when Eliommod_sessiongroups.Data.group_size
                    ...
                    if scope <> `Session then
                      Lwt_log.ign_error ~section
                        "Eliom: Group associated to IP has scope different \
                         from `Session. Please report the problem.";
                    Eliommod_sessiongroups.Data.remove session_group_node;
