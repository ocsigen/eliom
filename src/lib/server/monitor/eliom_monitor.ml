...
let format_duration t =
  ...
  let year =
    if tm.tm_year>0
    then (string_of_int (tm.tm_year - 70))^" years, "
    else "" in
...
let general_stats () =
  ...
  div [
    dl [
      ...
      dd [match fd ~pid with
          | `Error s -> ppf "Information on file descriptors not accessible (%s)." s
...
let gc_stats () =
  ...
  div [
    ul [
      ...
      li [ppf "%d words in the the major heap (max %d)." stat.Gc.heap_words stat.Gc.top_heap_words];
    ]
  ]
...
let lwt_stats () =
  div [
    ul [
      li [ppf "%d lwt threads waiting for inputs" (Lwt_engine.readable_count ()) ];
      li [ppf "%d lwt threads waiting for outputs" (Lwt_engine.writable_count ())];
...
let preemptive_thread_stats () =
  div [
    ul [
      li [ ppf "%d detached threads (min %d,max %d)."
      ...
      li [ ppf "%d computations queued (max %d)."
  ...
  div [
    ...
    ul (List.map (fun (vhosts,config,_) ->
        let all_vhost = String.concat ", " (List.map (fun (vhost,_,vport) ->
            ...
            Printf.sprintf "%s%s ( default: %s,  http: %d, https: %d )"
              vhost optport
              config.Ocsigen_extensions.default_hostname
...
let content_div () =
  ...
  Lwt.return (
    div [
      ...
      h2 [ppf "HTTP connexions"]; http_stats ();
      h2 [ppf "Eliom sessions"]; eliom_stats;
      h2 [ppf "GC"]; gc_stats ();
      h2 [ppf "Lwt threads"]; lwt_stats ();
      h2 [ppf "Preemptive threads"]; preemptive_thread_stats ();
    ])
  ...
  Lwt.return (
    html
      (head (title (pcdata "Server monitoring")) [
          ...
          link
            ~rel:[`Stylesheet]
            ~href:(uri_of_string (fun () -> "//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap-theme.min.css"))
            ()
        ])
      (body [
