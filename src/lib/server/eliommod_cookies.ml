  ...
  module Expiry_dates = struct
    ...
    let add_cookie exp cookie =
      ...
      | Some cookies_str ->
          ...
          if List.mem cookie cookies then Some cookies_str
    ...
    Eliom_common.Full_state_name_table.fold
      (fun name value (oktable, failedlist) ->
        try
          ...
          let {
            Eliom_common.Service_cookie.session_table;
            expiry;
            timeout;
            session_group;
            session_group_node;
          ...
          (fun () ->
            ...
            return
              ...
              | Some oldv, Some (_, None, exp) ->
                  if exp = Eliom_common.CENothing then beg
                  else
                    Ocsigen_cookie_map.add
                      ~path:(Eliom_common.get_site_dir sitedata)
