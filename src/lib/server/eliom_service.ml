...
let attach :
  ...
  fun ~fallback ~service () ->
    ...
    let get_name = match na_name with
      ...
      | SNa_post_ _s -> fallbackkind.get_name (*VVV check *)
      | SNa_post' _s -> fallbackkind.get_name (*VVV check *)
      | SNa_post_csrf_safe _a -> fallbackkind.get_name (*VVV check *)
