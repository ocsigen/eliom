...
let coordinates_to_string {abscissa;ordinate}= Printf.sprintf "%d_%d" abscissa ordinate
let coordinates_of_string s =
  let i = String.index s '_' in
  let a = String.sub s 0 i in
  let o = String.sub s (i+1) (String.length s - i -1) in
  {abscissa=int_of_string a;ordinate=int_of_string o}

let coordinates s = user_type
