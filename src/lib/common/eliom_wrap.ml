...
module Tbl = struct
  type t =
    { mutable size : int;        (* Size of the hash table *)
      mutable shift : int;       (* For hashing *)
      mutable occupancy : int;   (* How many elements have been inserted *)
      mutable obj : Obj.t array; (* Inserted blocks *)
      mutable idx : int array;   (* Corresponding indices *)
      mutable gc : int;          (* Last minor GC cycle where the
                                    table was accurate *)
  ...
  let reallocate resize tbl =
    ...
    let rec insert tbl h x idx =
      ...
      if y == none then begin
        tbl.obj.(h) <- x;
        tbl.idx.(h) <- idx
      end else if y == x then
        tbl.idx.(h) <- max idx tbl.idx.(h) (* Keep largest index *)
      else
        insert tbl ((h + 1) land (tbl.size - 1)) x idx
    in
  ...
  let rec allocate_rec tbl x i =
    if tbl.obj.(i) == x then
      tbl.idx.(i)
    else if tbl.obj.(i) == none then begin
      tbl.obj.(i) <- x;
      ...
      allocate_rec tbl x ((i + 1) land (tbl.size - 1))
  ...
  let rec get_rec tbl x i =
    ...
    if y == x then
      ...
      get_rec tbl x ((i + 1) land (tbl.size - 1))
...
let obj_kind v =
  if not (Obj.is_block v) then
    ...
    if tag >= Obj.no_scan_tag then
      ...
      `Scannable
    else if tag = Obj.forward_tag then begin
      let tag' = Obj.tag (Obj.field v 0) in
      if tag' = Obj.forward_tag || tag' = Obj.double_tag then
        `Scannable
      else
        (* Forward pointer that may be optimized away by the GC *)
        `Forward
    end else begin
...
let unchanged = (* This block and its descendants can be left unchanged *)
  Obj.repr 1
let modified =  (* This block or its descendants may need to be modified *)
  Obj.repr 2
...
let rec find_substs tbl subst_tbl v =
  ...
  | `Scannable ->
    ...
    if v' == none (* Not visited yet *) then begin
      if is_marked v then begin
        if not (Tbl.was_up_to_date tbl) then begin
...
let rec duplicate tbl subst_tbl copy_tbl orig =
  ...
  | `Scannable ->
    ...
    if subst == unchanged then
      (* This block does not need to be copied *)
      orig
...
let perform_wrap =
  ...
  Lwt_log.ign_debug_f ~section
    "Wrap stats: \
     %d visited (%d blocks), %d wrapped, %d copied, %d resizes, %d rehashes"
    !iteration_count tbl.occupancy !wrap_count !copy_count
    !resize_count !rehash_count;
  w
