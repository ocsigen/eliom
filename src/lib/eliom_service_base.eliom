...
let params_of_meth : type m gp gn pp pn x.
    ...
    (gp, 'tipo, gn) params * (pp, [ `WithoutSuffix ], pn) params = function
...
let which_meth_internal : type m gp gn pp pn tipo x.
    (m, gp, gn, pp, pn, tipo, x) meth -> m which_meth = function
  | Get _ -> Get'
  | Post _ -> Post'
  | Put _ -> Put'
  | Delete _ -> Delete'
...
let%server no_client_fun () : _ ref Eliom_client_value.t option =
  (* It only makes sense to create a client value when in a global
     context. *)
  if Eliom_syntax.global_context () then Some [%client.unsafe ref None]
  else None
