...
module ReactiveData = struct
  module RList = struct
    ...
    module Lwt = struct
      let map_data_p_lwt = Fiber.List.map
      ...
      let map_patch_p_lwt f = Fiber.List.map (map_patch_p_lwt f)
      ...
      let map_p (f : 'a -> 'b) (l : 'a t) : 'b t =
        ...
        let r_th =
          let r = Fiber.List.map f (ReactiveData.RList.value l) in
...
module React = struct
  module S = struct
    ...
    module Lwt = struct
      ...
      let l2_s ?eq (f : ('a -> 'b -> 'c) Value.t)
          (s1 : 'a FakeReact.S.t Value.t) (s2 : 'b FakeReact.S.t Value.t) : 'c t
          =
        ...
        Value.create
          (fst (FakeReact.S.create ~synced server_result))
          [%client.unsafe
            (React.S.Lwt.l2_s_init ~init:~%server_result ?eq:~%eq ~%f ~%s1 ~%s2
              : 'c FakeReact.S.t)]

      let l3_s ?eq (f : ('a -> 'b -> 'c -> 'd) Value.t)
          (s1 : 'a FakeReact.S.t Value.t) (s2 : 'b FakeReact.S.t Value.t)
          (s3 : 'c FakeReact.S.t Value.t) : 'd t =
        ...
        Value.create
          (fst (FakeReact.S.create ~synced server_result))
          [%client.unsafe
            (React.S.Lwt.l3_s_init ?eq:~%eq ~init:~%server_result ~%f ~%s1 ~%s2
               ~%s3
              : 'd FakeReact.S.t)]

      let l4_s ?eq (f : ('a -> 'b -> 'c -> 'd -> 'e) Value.t)
          (s1 : 'a FakeReact.S.t Value.t) (s2 : 'b FakeReact.S.t Value.t)
          (s3 : 'c FakeReact.S.t Value.t) (s4 : 'd FakeReact.S.t Value.t) : 'e t
          =
        let s1' = Value.local s1
        ...
        Value.create
          (fst (FakeReact.S.create ~synced server_result))
          [%client.unsafe
            (React.S.Lwt.l4_s_init ?eq:~%eq ~init:~%server_result ~%f ~%s1 ~%s2
               ~%s3 ~%s4
              : 'e FakeReact.S.t)]

      let l5_s ?eq (f : ('a -> 'b -> 'c -> 'd -> 'e -> 'f) Value.t)
          (s1 : 'a FakeReact.S.t Value.t) (s2 : 'b FakeReact.S.t Value.t)
          (s3 : 'c FakeReact.S.t Value.t) (s4 : 'd FakeReact.S.t Value.t)
          (s5 : 'e FakeReact.S.t Value.t) : 'f t =
        let s1' = Value.local s1
        ...
        Value.create
          (fst (FakeReact.S.create ~synced server_result))
          [%client.unsafe
            (React.S.Lwt.l5_s_init ?eq:~%eq ~init:~%server_result ~%f ~%s1 ~%s2
               ~%s3 ~%s4 ~%s5
              : 'f FakeReact.S.t)]

      let l6_s ?eq (f : ('a -> 'b -> 'c -> 'd -> 'e -> 'f -> 'g) Value.t)
          (s1 : 'a FakeReact.S.t Value.t) (s2 : 'b FakeReact.S.t Value.t)
          (s3 : 'c FakeReact.S.t Value.t) (s4 : 'd FakeReact.S.t Value.t)
          (s5 : 'e FakeReact.S.t Value.t) (s6 : 'f FakeReact.S.t Value.t) : 'g t
          =
        ...
        Value.create
          (fst (FakeReact.S.create ~synced server_result))
          [%client.unsafe
            (React.S.Lwt.l6_s_init ?eq:~%eq ~init:~%server_result ~%f ~%s1 ~%s2
               ~%s3 ~%s4 ~%s5 ~%s6
              : 'g FakeReact.S.t)]

      let merge_s ?eq (f : ('a -> 'b -> 'a) Value.t) (acc : 'a)
          (l : 'b FakeReact.S.t Value.t list) : 'a t =
...
module ReactiveData = struct
  module RList = struct
    ...
    module Lwt = struct
      let map_p (f : ('a -> 'b) Value.t)
          (l : 'a FakeReactiveData.RList.t Value.t) : 'b t =
        let l' = Value.local l in
        let server_result =
          Fiber.List.map (Value.local f) (FakeReactiveData.RList.value l')
        in
