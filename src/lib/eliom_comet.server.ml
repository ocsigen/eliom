...
let fallback_global_service =
  ...
  Comet.create ~meth:(Eliom_service.Get Eliom_parameter.unit)
    ~path:(Eliom_service.Path comet_global_path) (fun () () ->
      Lwt.return
        (error_msg
           "request with no post parameters, or there isn't any registered \
            site comet channel"))
...
module Stateless : sig
  ...
  let create ?(name = new_id ()) ~size stream =
    ...
    | Some _ ->
        failwith
          (Printf.sprintf
             "can't create channel %s: a channel with the same name already \
              exists"
             name)
    | None ->
  ...
  let queue_take channel last =
    try
      Dlist.fold
        (fun l (v, index) ->
          if index >= last then
            (channel.ch_id, Eliom_comet_base.Data (v, index)) :: l
  ...
  let wait_data requests =
    if List.exists has_data requests then Lwt.return_unit
    else
  ...
  let handle_request () (_, req) =
    ...
    | Eliom_comet_base.Stateful _ ->
        failwith
          "attempting to request data on stateless service with a stateful \
           request"
  ...
  let get_kind ~newest { ch_index; _ } =
    if newest then Eliom_comet_base.Newest_kind (ch_index + 1)
...
module Stateful : sig
  ...
  let wait_handler_timeout handler t =
    let rec run () =
      ...
      | Inactive inactive_time ->
          ...
          if now -. inactive_time > t then Lwt.return_unit
          else
    ...
    let rec take n acc channels =
      if n = 0 then acc
        ...
        | (id, Events { queue; _ }) :: rem ->
            if Queue.is_empty queue then take n acc rem
  ...
  let register_channel handler chan_id =
    ...
    if not (List.mem_assoc chan_id handler.hd_active_channels) then
      try
  ...
  let run_handler handler =
    let f () (idle, req) =
      ...
      | Eliom_comet_base.Stateless _ ->
          failwith
            "attempting to request data on stateful service with a stateless \
             request"
      | Eliom_comet_base.Stateful (Eliom_comet_base.Request_data number) ->
          ...
          if snd handler.hd_last = number then Lwt.return (fst handler.hd_last)
          else
            Lwt.catch
              (fun () ->
                Lwt_unix.with_timeout (timeout ()) (fun () ->
                    let* messages =
                      ...
                      if messages <> [] || idle then Lwt.return messages
                      else
                        let* () =
  ...
  let get_ref eref =
    ...
    | _ ->
        failwith
          "Eliom_comet: accessing channel references should not be blocking: \
           this is an eliom bug"
            ...
            (Obj.repr
               (React.E.fold
                  (fun full x ->
                    ...
                    full
                    ||
                    if Queue.length queue > size then (
                      channel.events <- None;
                      Queue.clear queue;
                      ...
                      false))
                  false events)));
    if List.mem name handler.hd_registered_chan_id then (
      handler.hd_registered_chan_id <-
