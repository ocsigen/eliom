...
module Syntax_helpers : sig
  ...
  (** Registers a client value datum for the next server section when
      ...
      (cf. {!Eliom_client_common2.Syntax_helpers.set_global}) or in the
      request_data when executed in a request. *)
  val client_value : ?pos:Eliom_lib.pos -> string -> 'args -> 'a Eliom_client_common.client_value
  ...
  (** Called at the end of each server or shared section. The argument
      ...
      {!Eliom_client_common.client_value_datum}s into the queue of server
  ...
  val close_client_section :
    string -> (int * Ocsigen_lib.poly * Eliom_lib.pos * string option) list -> unit

  (** Convert any value to a {! Eliom_client_common.escaped_value} for usage in
      the [args] argument to {! Eliom_client_common2.Syntax_helpers.client_value}. *)
