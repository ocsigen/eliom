...
let late_unwrap_value old_value new_value =
  ...
  (* Only process if the value has late-unwrap marker structure.
     ...
     If the last field doesn't have this structure, there are no occurrences
     to update, so we can skip. *)
  if size >= 1 then begin
    let last_field = Obj.field old_value (size - 1) in
    if Obj.is_block last_field && Obj.size last_field >= 3 then
