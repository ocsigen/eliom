...
module E = struct
  ...
  let from f =
    let event, push = create () in
    let rec loop () = f () >>= fun x -> push x; loop () in
  ...
  let accum_s ef acc =
    ...
    let iter =
      fmap
        (fun f ->
           Lwt.on_success
             (Lwt_mutex.with_lock mutex (fun () -> f !acc))
             (fun x ->
                acc := x;
                push x);
           None)
  ...
  let fold_s f acc e =
    ...
    let iter =
      fmap
        (fun x ->
           Lwt.on_success
             (Lwt_mutex.with_lock mutex (fun () -> f !acc x))
             (fun x ->
                acc := x;
                push x);
           None)
  ...
  let rec rev_fold f acc = function
    | [] -> Lwt.return acc
    | x :: l -> rev_fold f acc l >>= fun acc -> f acc x
...
module S = struct
  ...
  let filter_s ?eq f i s =
    ...
    | true -> Lwt.return (hold ?eq x (E.select [iter; event]))
    | false -> Lwt.return (hold ?eq i (E.select [iter; event]))
  ...
  let fmap_s ?eq f i s =
    ...
    | Some x -> Lwt.return (hold ?eq x (E.select [iter; event]))
    | None -> Lwt.return (hold ?eq i (E.select [iter; event]))
