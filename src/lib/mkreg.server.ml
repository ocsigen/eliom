...
(* This test check if there is a header set only by
   Registration.App. This test is sufficient, but it is better
...
let check_process_redir sp f param =
  let redir =
    ...
    then
      match sp.Common.sp_client_appl_name with
...
let register
  ...
  | None, None | Some `Site, None -> (
      let aux sitedata =
        (match S.info service with
        | S.Attached attser ->
            Common.remove_unregistered sitedata (S.sub_path attser)
        | S.Nonattached naser ->
      ...
      | Some get_current_sitedata ->
          ...
          if sitedata.Common.site_dir <> None
          then aux sitedata
          else
...
let create
  ...
  let service =
    S.create_unsafe ?name ?csrf_safe
      ?csrf_scope:(csrf_scope :> Common.user_scope option)
