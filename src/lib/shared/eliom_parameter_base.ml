...
let rec make_suffix : type a c. (a,'b,c) params_type -> a -> string list = fun typ params -> match typ with
  ...
  | TCoord _n -> (make_suffix (TAtom ("",TInt)) (params.abscissa))@
...
let rec walk_parameter_tree : type a c. string -> (a,'b,c) params_type -> a to_and_of option = fun name x ->
  ...
  match x with
    ...
    | TNLParams _ -> None
    | TUnit -> None
    | TOption (_o,_) -> failwith "walk_parameter_tree with option"
    | TSet _o -> failwith "walk_parameter_tree with set"
    | TList (_, _o) -> failwith "walk_parameter_tree with list"
    | TProd (_a, _b) -> failwith "walk_parameter_tree with tuple"
    | TSum (_a, _b) -> failwith "walk_parameter_tree with sum"
    ...
    match (typ, suff) with
      ...
      | TUserType (_name, tao), v::l ->
        (try
         ...
         with e -> raise (Eliom_common.Eliom_Typing_Error [("<suffix>", e)]))
      | TTypeFilter (_t, None), _ -> failwith "Type filter without filter"
      ...
      | TJson (_, None), _v::_l -> assert false (* client side only *)
  ...
  and aux : type a c. (a, 'b, c) params_type -> params' -> files -> string -> string -> a res_reconstr_param =
    fun typ params files pref suff ->
      match typ with
        ...
        | TAtom (name,TBool) ->
          (try
             let _v,l = (List.assoc_remove (pref^name^suff) params) in
             Res_ (true,l,files)
        ...
        | TAtom (name,a) ->
          ...
          (try
             Res_ (atom_of_string a v, l, files)
           with e ->
             Errors_ ([pref^name^suff, v, e], l, files))
        | TFile name ->
          (try
        ...
        | TTypeFilter (_t, None) -> failwith "Type filter without filter"
  ...
  try
    match aux typ params files "" "" with
      ...
      | Errors_ (errs, l, files) ->
        if (l, files) = ([], [])
        then raise (Eliom_common.Eliom_Typing_Error (List.map (fun (v,_l,e) -> (v,e)) errs))
        else raise Eliom_common.Eliom_Wrong_parameter
