...
let check_global_data global_data =
  ...
  | l ->
      Printf.ksprintf
        ...
        "Code generating the following client values is not linked on the \
         client:\n\
         %s"
        (String.concat "\n"
           (List.rev_map
...
let form_handler :
  ...
  Dom_html.full_handler (fun node ev ->
      ...
      in
      let kind =
        if String.lowercase_ascii (Js.to_string form##._method) = "get" then
          `Form_get
        else `Form_post
...
let relink_process_node (node : Dom_html.element Js.t) =
  ...
  Js.Optdef.case
    ...
    (fun pnode ->
      ...
      if String.sub (Js.to_bytestring id) 0 7 <> "global_" then (
...
let relink_request_nodes root =
  ...
  if !Eliom_config.debug_timings then
    Console.console##(time (Js.string "relink_request_nodes"));
  Eliommod_dom.iter_nodeList
    (Eliommod_dom.select_request_nodes root)
    relink_request_node;
  if !Eliom_config.debug_timings then
    Console.console##(timeEnd (Js.string "relink_request_nodes"))
...
let relink_closure_node root onload table (node : Dom_html.element Js.t) =
  ...
  let aux attr =
    if is_closure_attrib attr then
...
let load_data_script page =
  ...
  let data_script : Dom_html.scriptElement Js.t =
    ...
    | _ :: _ :: data_script :: _ -> (
        ...
        | t ->
            raise_error ~section
              "Unable to find Eliom application data (script element expected, \
               found %s element)"
              t)
...
module History = struct
  ...
  let replace n =
    let maybe_replace p =
      if p.page_id.state_index = n.page_id.state_index then (
...
let init () =
  ...
  let onload _ev =
    ...
    Lwt.async (fun () ->
        if !Eliom_config.debug_timings then
          Console.console##(time (Js.string "onload"));
        let* () =
        ...
        if !Eliom_config.debug_timings then
          Console.console##(timeEnd (Js.string "onload"));
        Lwt.return_unit);
    Js._false
  in
  Lwt_log.ign_debug ~section "Set load/onload events";
  if Dom_html.document##.readyState = Js.string "complete" then
    Lwt.async @@ fun () ->
...
let push_history_dom () =
  if !dom_history_ready then (
    let page = !active_page in
    let dom =
      if !only_replace_body then Dom_html.document##.body
...
module Page_status = struct
  ...
  let onactive ?(now = true) ?(once = false) ?stop action =
    let on_event () =
      ...
      @@ Events.active ()
    in
    if now && React.S.value (signal ()) = Active then (
      action ();
  ...
  if Eliom_process.history_api then (
    ...
    if replace then (
      ...
      Dom_html.window##.history##replaceState
        ...
        (Js.string "")
        (if !Eliom_common.is_client_app then Js.null
    ...
    if uri <> fst (Url.split_fragment Url.Current.as_string) then
...
let replace_page ~do_insert_base new_page =
  if !Eliom_config.debug_timings then
    Console.console##(time (Js.string "replace_page"));
  if !only_replace_body then
    ...
    Dom.replaceChild Dom_html.document new_page
      Dom_html.document##.documentElement);
  if !Eliom_config.debug_timings then
    Console.console##(timeEnd (Js.string "replace_page"))
...
let set_content_local ?offset ?fragment new_page =
  ...
  Lwt.catch
    (fun () ->
      ...
      Eliom_client_core.set_loading_phase ();
      if !Eliom_config.debug_timings then
        Console.console##(time (Js.string "set_content_local"));
...
let set_content ~replace ~uri ?offset ?fragment content =
  ...
  | Some content ->
      ...
      let really_set () =
        ...
        advance_page ();
        if !Eliom_config.debug_timings then
          Console.console##(timeEnd (Js.string "set_content"));
        Lwt.return_unit
      and recover () =
        if !locked then Lwt_mutex.unlock Eliom_client_core.load_mutex;
        if !Eliom_config.debug_timings then
      ...
      Lwt.catch
        (fun () ->
          ...
          Eliom_client_core.set_loading_phase ();
          if !Eliom_config.debug_timings then
            Console.console##(time (Js.string "set_content"));
          let g () =
            recover ();
            Lwt.return_unit
          in
...
let rec handle_result ~replace ~uri result =
  ...
  | Reload_action { hidden; https } -> (
      ...
      | true, true ->
          switch_to_https ();
          reload ~replace ~uri
            ~fallback:Eliom_service.reload_action_https_hidden)
    ...
    with_progress_cursor
      ...
      | _ -> (
          ...
          | Some f when not ignore_client_fun ->
              Lwt_log.ign_debug ~section:section_page
                "change page: client_fun service is Some and (not \
                 ignore_client_fun)";
          ...
          | None when is_client_app () ->
              ...
              Lwt.return
              @@ exit_to ?absolute ?absolute_path ?https ~service ?hostname
...
let change_page_uri_a ?cookies_info ?tmpl ?(get_params = []) full_uri =
  ...
  with_progress_cursor
    (let uri, fragment = Url.split_fragment full_uri in
     if uri <> get_current_uri () || fragment = None then (
       if is_client_app () then
    ...
    let auto_change_page fragment =
      Lwt.ignore_result
        (let l = String.length fragment in
         if l = 0 || (l > 1 && fragment.[1] = '!') then
           if fragment <> !current_pseudo_fragment then (
