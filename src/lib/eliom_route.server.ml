              ...
              | (Eliom_common.Eliom_404 | Eliom_common.Eliom_Wrong_parameter) as
                exn -> (
                      ...
                      let request =
                        {
                          ri with
                          request_info =
                            Ocsigen_request.update ri.request_info
                              ~post_data:None ~meth:`GET;
                        }
                      and session_info =
                        {
                          si with
                  ...
                  | Eliom_common.RAtt_anon _, Eliom_common.RAtt_no ->
                    ...
                    *)
                      Lwt_log.ign_info ~section
                        "Link to old. Trying without GET state parameters and \
                         POST parameters:";
                      Polytables.set
                        ~table:(Ocsigen_request.request_cache ri.request_info)
                        ~key:Eliom_common.eliom_link_too_old ~value:true;
...
let add_naservice tables name (max_use, expdate, naservice) =
  ...
  (if sp = None (* not duringsession *) then
     try
       let g, _, _, _, _ =
...
let make_naservice now
  ...
  (try
     ...
     with Not_found ->
       Lwt_log.ign_info ~section
         "Looking for a non attached service in the global table";
       return
   ...
   with Not_found -> (
     ...
     *)
     ...
     | Eliom_common.RNa_post_ _ | Eliom_common.RNa_post' _ ->
         ...
         Lwt_log.ign_info ~section
           "Link too old to a non-attached POST coservice. Try without POST \
            parameters:";
         Polytables.set
           ~table:(Ocsigen_request.request_cache ri.request_info)
           ~key:Eliom_common.eliom_link_too_old ~value:true;
