...
The eliom syntax extension allows to write, in one file, both the client and server part of your application. The extension is automatically enabled by the [eliomc] and [js_of_eliomc] compilers on [.eliom] and [.eliomi] files.
...
thereafter refer to it: an expression placed inside [[%client ... ]] will be computed
...
For example, here, the expression [1 + 3] will be evaluated on the client, but it’s
...
In this case, [x] has type [int fragment].
...
{2 Injections and escaped values}

Values that have been computed on the server can be used on the client by
prefixing them with a percent symbol [~%]. We call this an {i injection}.
...
Here, the expression [1 + 2] is evaluated and bound to variable [s] on the server.
The resulting value [3] is transferred to the client together with the Web page,
then the expression [~%s + 1] is computed client-side.
