...
module Pass = struct
  ...
  (* Replace every escaped identifier [v] with [Eliom_client0.Syntax_helpers.get_escaped_value v] *)
      ...
      {Ast_mapper.default_mapper with
       expr = (fun mapper e ->
         ...
         | Pexp_ident {txt} when Mli.is_escaped_ident @@ Longident.last txt ->
           [%expr Eliom_client0.Syntax_helpers.get_escaped_value [%e e] ]
           [@metaloc e.pexp_loc]
  ...
  let escape_inject ?ident ~(context:Context.escape_inject) ~id expr =
    ...
    | `Injection _section ->
      ...
      [%expr
        (Eliom_client0.Syntax_helpers.get_injection
           ?ident:([%e ident])
           ~pos:([%e position loc])
           [%e es]
         : [%t typ])
