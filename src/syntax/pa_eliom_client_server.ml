...
module Server_pass(Helpers : Pa_eliom_seed.Helpers) = struct
  ...
  let push_escaped_binding, flush_escaped_bindings =
    ...
    let flush () =
      ...
      let aux (_, arg) =
        ...
        <:expr< Eliom_client_common2.Syntax_helpers.escaped_value $arg$ >>
      in
      List.map aux res
    in
    push, flush
  ...
  let close_server_section loc =
    ...
    <:str_item<
        let () =
          Eliom_client_common2.Syntax_helpers.close_server_section
  ...
  let close_client_section loc injections =
    ...
    <:str_item<
        let () =
          Eliom_client_common2.Syntax_helpers.close_client_section
            $str:Helpers.file_hash loc$
            $injection_list$
    >>
  ...
  let client_value_expr typ context_level orig_expr gen_id _ loc =
    ...
    in
    <:expr@loc<
      (Eliom_client_common2.Syntax_helpers.client_value
  ...
  let implem loc sil =
    let _loc = Loc.ghost in
    let set_global b =
      <:str_item< let () = Eliom_client_common2.Syntax_helpers.set_global $`bool:b$ >>
    in
