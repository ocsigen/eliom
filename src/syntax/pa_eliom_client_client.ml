...
(* BB {2 map_get_escaped_values / escape_inject generates only $lid:gen_id$}
   ...
   Only for the former it is necessary to call [Eliom_client_core.Syntax_helpers.get_escaped_value]
  ...
  let escape_inject context_level ?ident orig_expr gen_id =
    ...
    match context_level with
      ...
      | Injected_in _section ->
          ...
          <:expr<
            (Eliom_client_core.Syntax_helpers.get_injection ?ident:($ident$) ~pos:($Helpers.position _loc$) $str:s$ : $typ$)
          >>
