...
let usage () =
  ...
  if !kind =  `Server || !kind = `ServerOpt then begin
    ...
    Printf.eprintf "  -server-dir <opt>\t\tThe directory for the files generated by \
      eliomc (default %S, necessary if js_of_eliom is called without -noinfer and \
      eliomc was used with -dir).\n"
      default_server_dir;
  end;
...
let compile_server_type_eliom file =
  ...
  create_process ~out !compiler ( [ "-i" ; "-pp"; get_pp ppopts]
				  @ !args
				  @ get_common_include ~kind:`Server ~build_dir ()
				  @ ["-impl"; file] );
  Unix.close out
...
let rec process_option () =
  ...
  while !i < Array.length Sys.argv do
    ...
    | "-type-dir" ->
      ...
      i := !i+2
    | "-server-dir" ->
      if !kind <> `Client || !i+1 >= Array.length Sys.argv then usage ();
      server_dir := Some Sys.argv.(!i+1);
      i := !i+2
    | "-package" ->
      if !i+1 >= Array.length Sys.argv then usage ();
