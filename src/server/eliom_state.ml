...
(*VVV!!! est-ce que sgr_o est fullsessgrp ? *)
  ...
  let get_service_scope_name ~cookie:(_, ((_, s), _, _, _, _, _)) =
    split_name s

  let get_volatile_data_scope_name ~cookie:(_, ((_, s), _, _, _, _)) =
    split_name s

  let get_persistent_data_scope_name ~cookie:(_, ((_, s), _, _, _)) =
    split_name s
