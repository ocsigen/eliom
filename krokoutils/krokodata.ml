...
module Rights : sig
  ...
  module UsersCache = Krokache.Make(
    struct
	...
	| _ -> raise (Krokache.Cache_error "users (probably database table wrong?)")
  ...
  module ResourceCache = Krokache.Make(
    struct
	...
	| _ -> raise (Krokache.Cache_error "resources (probably database table wrong?)")
  ...
  let rec get_passwd () = 
    let pwd1 =
      Printf.printf "Enter Krokoutils admin password:%!";
      Scanf.scanf "%s" (fun x -> x)
    in
    let pwd2 =
      Printf.printf "Enter Krokoutils admin password once again:%!";
      Scanf.scanf "%s" (fun x -> x)
    in if pwd1 = pwd2 
    then pwd1 
    else begin
  ...
  let root_uid =
    Krokopersist.make_persistant_lazy 
      "root_uid"
      (fun () -> 
	UsersCache.insert 
  ...
  let anonymoususer = Krokopersist.get anonymoususer_uid
