...
let parse_config path charset parse_fun =
  ...
  in
  function
    | Element (("allow" | "deny"), attrs, sub) as e -> gen (parse_sub e)
    | Element ("forbidden", [], []) -> gen (Filter_Error (Ocsigen_http_error 403))
    | Element ("notfound", [], []) -> gen (Filter_Error (Ocsigen_http_error 404))
    | Element ("iffound", [], sub) ->
