...
module Uri :
  sig
    type t;
    value uri_of_string : string -> t;
    value string_of_uri : t -> string;
  end =
  struct
    type t = string;
    value uri_of_string s = s;
    value string_of_uri s = s;
  end;
...
type multilengths = list multilength ; (* comma-separated *)
    (** A comma separated list of items of type MultiLength. *)

type number = int;
type numbers = list number; (* space-separated *)
...
type uri = Uri.t; (* I abstract this for Ocsigen -- VB *)
    (** A Uniform Resource Identifier, as per RFC2396.
        @see <http://www.ietf.org/rfc/rfc2396.txt> RFC2396 *)
value uri_of_string = Uri.uri_of_string;
value string_of_uri = Uri.string_of_uri;
...
type phrasing_without_progress = 
  [=resetable | submitable | `Wbr | `Var | `Time | `Sup | `Sub | `Strong 
  | `Span | `Small | `Script | `Samp | `Ruby | `Q | `Meter | `Mark 
  |`Label| `Kbd |  `I |`Em | `Dfn |  `Datalist | `Command | `Code | `Cite 
  |`Button | `Br | `Bdo | `B | `Abbr | `PCDATA
  | transparent
      phrasing_without_interactive
      phrasing_without_noscript
...
type phrasing_without_time = 
  [= labelable | submitable | `Wbr | `Var | `Sup | `Sub | `Strong | `Span 
  | `Small | `Script | `Samp | `Ruby | `Q | `Mark |`Label| `Kbd 
  |  `I |`Em | `Dfn |  `Datalist | `Command | `Code | `Cite | `Br | `Bdo 
  | `B | `Abbr | `PCDATA 
  | transparent
      phrasing_without_interactive
      phrasing_without_noscript
...
type core_flow5 = 
  [= core_phrasing | formassociated | formatblock | `Ul | `Table |`Style | `Ol 
  | `Menu | `Hr | `Form |`Figure|  `Dl ];

type flow5_without_interactive = 
  [= core_flow5 | transparent_without_interactive flow5_without_interactive  ];
...
type flow5_without_media = 
  ...
  | `Keygen | `Input | `Img of on_off | `Iframe | `Embed | `Details | `Button
  | formassociated | formatblock | `Ul | `Table |`Style | `Ol | `Menu | `Hr 
  | `Form |`Figure | `Dl|  transparent_without_media flow5_without_media];
...
type flow5_without_table = 
  [= core_phrasing | formassociated | formatblock | `Ul |`Style 
...
type flow5_without_form = 
  [= core_phrasing | formassociated | formatblock | `Ul | `Table |`Style 
  | `Ol | `Menu |  `Hr |`Figure | `Dl
  | transparent 
      flow5_without_interactive
      flow5_without_noscript
      flow5
...
type flow5_without_sectioning_heading_header_footer_address = 
  ...
  | `Div | `Blockquote| `Ul | `Table |`Style | `Ol | `Menu
  | `Hr | `Form |`Figure | `Dl
  | transparent flow5_without_interactive
      flow5_without_noscript
      flow5
      flow5_without_media
  ];
...
type body = [= `Body ];
type body_attrib = 
  [= common | `OnAfterPrint | `OnBeforePrint | `OneBeforeUnload
  | `OnHashChange
...
type a_attrib = [= common | `Href | `Hreflang | `Media | `Rel | `Target | `Mime_type ];
...
type iframe_attrib = [= common | `Src (*| `Srcdoc*) | `Name | `Sandbox | `Seamless | `Width | `Height ];

type object__content = [= flow5 | `Param ];
type object__content_fun = flow5;
type object_ = [= `Object of (object__content * on_off) ];
type object__attrib = [= common | `Data | `Form | `Mime_type | `Height | `Width | `Name | `Usemap ];
...
type audio_attrib = [= common |`Poster |`Preload |`Autoplay |`Loop |`Controls |`Width |`Height];
...
type video_attrib = [= common |`Poster |`Preload |`Autoplay |`Loop |`Controls |`Width |`Height];
...
type area_attrib = [= common | `Alt | `Coords | `Shape| `Target | `Rel | `Media| `Hreflang | `Mime_type];
...
type form_attrib = [= common |`Accept_charset | `Action | `Enctype | `Method | `Name | `Target | `Autocomplete | `Novalidate ];
...
type button_attrib =
  [ = common | `Autofocus | `Disabled | `Form | `Formaction
...
type select_attrib = [= common |`Autofocus | `Multiple | `Name | `Size | `Form | `Disabled ];
...
type keygen_attrib = [= common | `Autofcus | `Challenge | `Disabled | `Form | `Keytype | `Name ];
...
type meter_attrib = [= common |`Float_Value |`Min |`Max |`Low |`High |`Optimum |`Form];
...
type command_attrib = [= common |`Icon |`Disabled |`Checked|`Radiogroup |`Command_Type];
...
type script = [= `Script ];
type script_attrib = [= common | `Async | `Charset | `Src | `Defer |`Mime_type ];
...
type link_attrib = [= common | `Hreflang | `Media | `Rel | `Href | `Sizes | `Mime_type ];
