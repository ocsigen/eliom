...
module Html5 = Html5.F
let launchtime = Unix.time ()
let main = Http.service [] any ()

let num_or_none = function
  | None -> "???"
  ...
  Eliom_registration.Html5.register
    ...
    (fun _ () ->
       ...
       let fd =
         ...
         with _ -> None
       in
       let nssess = Eliom_state.get_service_session_group_size () in
       let ndsess = Eliom_state.get_volatile_data_session_group_size () in
       let ntables = None (* Eliom_state.number_of_tables () *) in
...
<<
 <html>
   ...
   <body>
     ...
     <p>Number of
        $str:"" (* string_of_int (Ocsigen_http_com.Timeout.nb_threads_waiting_timeout ()) *)$
     connexions waiting for timeout: not implemented in that version</p>
     <h2>Eliom sessions</h2>
     <p>There are $str:num_or_none nssess$ Eliom service sessions opened,
        $str:num_or_none ndsess$ Eliom in memory data sessions opened.<br/>
     There are $str:num_or_none ntables$ Eliom in memory tables created $list:
      (match ntableselts with
      | [] -> [](* dot *)
      | a::l -> [](* list1 a l *))
       $
       </p>
     <p>There are $str:num_or_none None (* nbperssess *)$
        Eliom persistent sessions opened.<br/>
     and $str:num_or_none None (* nbperstab *)$ Eliom
    persistent tables created $list:
      ...
      <ul>
        <li>Service sessions: $str: num_or_none None (* (Eliommod_sessiongroups.Serv.nb_of_groups ()) *)$</li>
        <li>Volatile data sessions: $str: num_or_none None (* (Eliommod_sessiongroups.Data.nb_of_groups ()) *)$</li>
        <li>Persistent data sessions: $str: num_or_none None (* persgrplength *) $</li>
      </ul>
   </body>
 </html>
>>)
