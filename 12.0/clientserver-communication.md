...
RPCs are just syntactic sugar for pathless services returning OCaml values. The PPX syntax extension translates the function definition into a [`Eliom_client.server_function`](./eliom.server/Eliom_client.md#type-server_function).
...
```
A function `'a -> 'b Lwt.t` can be wrapped on the server by [`Eliom_client.server_function`](./eliom.server/Eliom_client.md#type-server_function). When the result is injected to the client, it appears as a plain function `'a -> 'b Lwt`.
...
Create a module for each kind of notifications you want using the [`Eliom_notif.Make_Simple`](./eliom.server/Eliom_notif-Make_Simple.md) functor. For example, there would be a module for new message notifications in a chat.

Each client process must subscribe to each resource for which it wants to receive the notifications (each chat currently opened, in our example), using function [`Eliom_notif.Make_Simple.listen`](./eliom.server/Eliom_notif-Make_Simple.md#val-listen).
...
These services are registered using [`Eliom_registration.Ocaml`](./eliom.server/Eliom_registration-Ocaml.md) and can be called using `Eliom_client.call_ocaml_service`.
...
The client can send OCaml values as parameters to services. To do that, declare the expected parameter type using `Eliom_parameter.ocaml` (see [`Eliom_parameter_sigs.S.ocaml`](./eliom.server/Eliom_parameter_sigs-module-type-S.md#type-ocaml)).
...
The simple low-level version above which all other following mechanisms are implemented is provided in the [`Eliom_comet.Channel`](./eliom.server/Eliom_comet-Channel.md) module.
...
- Channels created with scope [`Eliom_common.site_scope`](./eliom.server/Eliom_common.md#type-site_scope) or using [`Eliom_comet.Channel.create_newest`](./eliom.server/Eliom_comet-Channel.md#val-create_newest) are stateless channels: the memory consumption does not depend on the number of users requesting data on it. When the channels are not reachable from the server code, they are garbage-collected and closed. Named stateless channels can be accessed from `other servers`.
- Channels created with scope [`Eliom_common.default_process_scope`](./eliom.server/Eliom_common.md#val-default_process_scope) must be created inside a service handler. They are assigned to a particular client process. Different channels created with the same stream do not share memory. They are closed when requested or when the client process is closed. It is possible to know when a client stop requesting data on those channels using [`Eliom_comet.Channel.wait_timeout`](./eliom.server/Eliom_comet-Channel.md#val-wait_timeout). Be careful about memory consumption when using client process channels.
...
To share a React event or signal with the client, use functions [`Eliom_react.Down.of_react`](./eliom.server/Eliom_react-Down.md#val-of_react) or [`Eliom_react.S.Down.of_react`](./eliom.server/Eliom_react-S-Down.md#val-of_react)

On client-side, the value returned by those functions is directly a React event or signal.

The opposite is also available using [`Eliom_react.Up.create`](./eliom.server/Eliom_react-Up.md#val-create).
