...
```
The provided value serves as a default value for the optional parameter `~xhr` in the functions `Eliom_registration.*.{a, get_form, post_form, lwt_get_form, lwt_post_form}` (cf. `Eliom_registration.Html.a` et al.). This value can also be set in the [config file](http://ocsigen.org/eliom/dev/manual/config#h5o-25).
